...
function formatResultProse(value) {
	...
	if (status === "complete") footer = "\n---\n[✓ 本次所选页面的提取 Markdown 已完整提供，可直接用于回答。Complete document content delivered above. Artifact retained at: " + value.manifest_path + "]";
		...
		footer = "\n---\n[⚠ 正文未完整提供（受输出限制截断 / Content truncated to output limit）。" + (mdPath !== void 0 ? "Full markdown artifact at: " + mdPath + resumeInfo + "." : "Full markdown artifact path unavailable.") + " Result manifest: " + value.manifest_path + "]";
...
var MinerUService = class {
	...
	async projectBatch(rawItems, limit) {
		...
		const finalResults = rawItems.map((item) => {
			if (item.state === "failed") return item.failureView;
		...
		while (JSON.stringify(batchView).length > limit || formatParseDocumentProse(batchView).length > limit) {
			...
			if (candidates.length > 0) {
				...
				const cut = truncateAtCleanBoundary(fileTexts.get(target.files[0]?.file_id ?? "")?.text ?? target.markdown_content, targetLen);
				const updatedTarget = {
					...target,
					content_status: "partial",
					markdown_content: cut.text,
...
var ProcessLock = class {
	...
	async acquire(signal) {
		...
		const socketName = process.platform === "win32" ? `\\\\.\\pipe\\dsh-pdf-mineru-${createHash("sha256").update((await realpath(this.paths.root)).toLowerCase()).digest("hex").slice(0, 32)}` : this.socketName;
		...
		this.server = server;
		try {
			if (process.platform === "win32") await this.acquireWindowsMetadata(payload, signal);
			else await writeFile(this.lockFilePath, JSON.stringify(payload, null, 2), {
				flag: "w",
				mode: 384
			});
	...
	async acquireWindowsMetadata(payload, signal) {
	...
	async reclaimDeadWindowsMetadata() {
		try {
			...
			if (!after.isFile() || after.isSymbolicLink() || before.dev !== after.dev || before.ino !== after.ino || before.mtimeMs !== after.mtimeMs || await readFile(this.lockFilePath, "utf8") !== raw) throw new Error("lock changed");
		...
		output: {
			...
			presentationMeta: (_args, value) => {
				...
				return {
					...
					...probe.server_version !== void 0 ? { server_version: probe.server_version } : {}
				};
			}
		},
...
//#endregion
export { ARTIFACT_KINDS, CACHE_KEY_SPEC_VERSION, CANONICAL_PARSE_REQUEST_SCHEMA_VERSION, Config, DEFAULT_RETRY_POLICY, MINERU_RESULT_MANIFEST_SCHEMA_VERSION, MinerUError, MinerUService, OfficialV4Provider, RESULT_SCHEMA_VERSION, SelfHostedV2Provider, StorageMaintenanceService, allocateReclaimedShares, apply, asCacheKey, asFileId, asOperationId, asProviderConfigId, asResultId, asSessionId, assertSafePathSegment, calculateBackoffDelay, createFileId, createOperationId, createStructuredDiagnosticSink, defaultMinerUConfig, defaultProviderConfig, defaultSleep, emitDiagnostic, executeWithRetry, failure, findMarkdownArtifactPath, formatParseDocumentProse, formatResultProse, inject, isRetryableError, isRetryableHttpStatus, mergeRetryOptions, migrateConfig, name, normalizeArtifactKinds, normalizePageRanges, parseRetryAfter, providerById, readBoundedResponseText, readMarkdownFile, resolveRetryPolicy, resultIdForCacheKey, safeStringSlice, sanitizeDiagnostic, throwMinerU, toMinerUFailure, truncateAtCleanBoundary, validateProviderCapabilities };
