...
var MinerUService = class {
	...
	async projectBatch(rawItems, limit) {
		...
		const fullTexts = /* @__PURE__ */ new Map();
		for (const item of completedItems) if (item.markdownRequested && item.markdownPath) {
		...
		const shares = allocateReclaimedShares(mdItems.map((i) => fileTexts.get(i.fileId)?.text.length ?? 0), totalTextBudget);
		const finalResults = await Promise.all(rawItems.map(async (item) => {
			...
			const raw = fileTexts.get(item.fileId) ?? {
				text: "",
				isCompleteFile: true
			};
			const markdownArtifact = {
			...
			if (raw.text.length <= share && raw.isCompleteFile) {
				...
				let full = raw.text;
				if (!raw.isCompleteFile && item.markdownPath) try {
		...
		while (JSON.stringify(batchView).length > limit || formatParseDocumentProse(batchView).length > limit) {
			...
			const candidates = batchView.results.filter((r) => r.state === "completed" && Boolean(r.markdown_content && r.markdown_content.length > 0));
...
function parseInput(args) {
	...
	const { poll_timeout_ms: rawPollTimeout, inline_images: rawInlineImages, max_inline_images: rawMaxInlineImages, ...input } = args;
	const pollTimeoutMs = parsePollTimeout(rawPollTimeout);
	let inline_images;
	if (rawInlineImages !== void 0) {
		if (typeof rawInlineImages !== "boolean") throw new MinerUError(failure("INVALID_REQUEST", "inline_images must be a boolean"));
		inline_images = rawInlineImages;
	}
	let max_inline_images;
	if (rawMaxInlineImages !== void 0) {
		if (!Number.isSafeInteger(rawMaxInlineImages) || rawMaxInlineImages <= 0) throw new MinerUError(failure("INVALID_REQUEST", "max_inline_images must be a positive integer"));
	...
	return {
		...
		...max_inline_images === void 0 ? {} : { max_inline_images }
	};
}
...
async function inlineImagesForSingleResult(view, attachments, maxInline, signal) {
	const candidates = await collectImageCandidates(view);
	if (candidates.length === 0) return view;
	candidates.sort((a, b) => b.bytes - a.bytes);
	const filtered = candidates.some((c) => c.bytes >= MIN_FIGURE_BYTES) ? candidates.filter((c) => c.bytes >= MIN_FIGURE_BYTES) : candidates;
...
async function processInlineImages(view, attachments, maxInline, signal) {
	const totalMax = Math.max(1, Math.min(10, typeof maxInline === "number" && Number.isSafeInteger(maxInline) ? maxInline : 3));
	if (!("kind" in view)) return await inlineImagesForSingleResult(view, attachments, totalMax, signal);
	let remaining = totalMax;
	const updatedResults = [];
	for (const res of view.results) if (res.state === "completed" && remaining > 0) {
...
function registerTools(ctx, getService, accessGate) {
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const effectiveArtifacts = input.artifacts ? shouldInline && !input.artifacts.includes("images") ? [...input.artifacts, "images"] : input.artifacts : shouldInline ? ["markdown", "images"] : void 0;
			const effectiveInput = effectiveArtifacts ? {
				...input,
			...
			if (shouldInline && attachments) return await processInlineImages(rawResult, attachments, max_inline_images, exec.signal);
			return rawResult;
		}
	})));
	return async () => {
...
//#endregion
export { ARTIFACT_KINDS, CACHE_KEY_SPEC_VERSION, CANONICAL_PARSE_REQUEST_SCHEMA_VERSION, Config, DEFAULT_RETRY_POLICY, MINERU_RESULT_MANIFEST_SCHEMA_VERSION, MinerUError, MinerUService, OfficialV4Provider, ProviderHttpClient, RESULT_SCHEMA_VERSION, SelfHostedV2Provider, StorageMaintenanceService, allocateReclaimedShares, apply, asCacheKey, asFileId, asOperationId, asProviderConfigId, asResultId, asSessionId, assertSafePathSegment, cacheClearConfirmationToken, calculateBackoffDelay, confirmationToken, createFileId, createHttpStatusError, createOperationId, createStructuredDiagnosticSink, defaultMinerUConfig, defaultProviderConfig, defaultSleep, emitDiagnostic, executeJsonRequest, executeWithRetry, extractErrorMessage, extractMarkdownHeadings, failure, findMarkdownArtifactPath, formatParseDocumentProse, formatResultProse, inject, isRetryableError, isRetryableHttpStatus, mergeRetryOptions, migrateConfig, name, normalizeArtifactKinds, normalizePageRanges, parseRetryAfter, providerById, readBoundedResponseText, readMarkdownFile, resolveProviderUrl, resolveRetryPolicy, resultIdForCacheKey, safeStringSlice, sanitizeDiagnostic, throwMinerU, toMinerUFailure, truncateAtCleanBoundary, validateProviderCapabilities };
