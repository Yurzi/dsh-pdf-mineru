...
import { chmod, link, mkdir, open, readFile, readdir, rename, rm, stat, unlink, writeFile } from "node:fs/promises";
...
function defaultMinerUConfig() {
	...
	return {
		...
		storage: {
			storageRoot: join(dshHome(), "dsh-pdf-mineru", "v1"),
			cacheEnabled: true,
			retainSources: false,
			stagingTtlMs: 864e5
		},
...
function migrateConfig(value) {
	...
	if (input.providers !== void 0 || input.activeProvider !== void 0) return parseCanonical(input, fallback);
	...
	return parseCanonical({
		...
		providers: [{
			...
			modelMap: {
				pipeline: defaultBackend === "pipeline" ? defaultBackend : "pipeline",
				vlm: vlmBackend
			},
...
function sanitizeDiagnostic(input) {
	return input.replace(/Bearer\s+[A-Za-z0-9._~+\/-]+/gi, "Bearer [REDACTED]").replace(/https?:\/\/[^\s<>"']+/gi, (raw) => {
		try {
...
var SelfHostedV2Provider = class {
	...
	async compatibilityKey(request, context) {
		...
		return `self-hosted-v2:${createHash("sha256").update(originAndPath, "utf8").digest("hex").slice(0, 16)}:${(context.configuredVersion ?? this.config.configuredVersion ?? this.config.id ?? "v2").replace(/[^A-Za-z0-9._-]/g, "_")}:${request.semantics.model}`;
	}
	async probe(context) {
		try {
	...
	async submit(request, sources, context) {
		...
		if (request.semantics.pages !== void 0) {
			...
			if (intervals.length === 1 && intervals[0] !== void 0) {
				form.append("start_page_id", String(intervals[0].start - 1));
				form.append("end_page_id", String(intervals[0].end - 1));
			} else throw new MinerUError(failure("UNSUPPORTED_OPTION", "Self-hosted v2 provider only supports a single continuous page range"));
		}
		...
		const fileSnapshots = request.files.map((f) => ({
			fileId: f.fileId,
			state: state === "queued" ? "queued" : state === "processing" ? "processing" : state === "completed" ? "completed" : "failed",
			rawState: data.status,
			failure: state === "failed" ? failure("REMOTE_PARSE_FAILED", data.error ?? "Remote task submission failed", false, {
				provider: "self-hosted-v2",
				fileId: f.fileId
			}) : void 0
		}));
	...
	async inspect(ref, context) {
		...
		const fileFailure = state === "failed" ? failure("REMOTE_PARSE_FAILED", data.error ?? "Remote task failed", false, { provider: "self-hosted-v2" }) : void 0;
		return {
			state,
	...
	async collect(ref, request, sink, context) {
		...
		for (const file of ref.files) {
			...
			if (artifacts.length === 0) collectedFiles.push({
				fileId: file.fileId,
				name: file.name,
				artifacts: [],
				failure: failure("REMOTE_PARSE_FAILED", `No matching artifacts produced for file "${file.name}"`, false, {
					provider: "self-hosted-v2",
					fileId: file.fileId
				})
			});
	...
	async requestJson(method, path, bodyStream, headers, context, acceptedStatuses = [200]) {
		context.signal.throwIfAborted();
		...
		try {
			...
			try {
				...
				if (timedOut) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", `Request to MinerU server timed out after ${context.timeoutMs}ms`, true));
			...
			if (!acceptedStatuses.includes(status)) {
				...
				if (status === 401 || status === 403) throw new MinerUError(failure("AUTHENTICATION_FAILED", `Authentication failed (${status})${diagnostic}`, false, { provider: "self-hosted-v2" }));
				if (status === 404) throw new MinerUError(failure("JOB_NOT_FOUND", `Resource not found (${status})${diagnostic}`, false, { provider: "self-hosted-v2" }));
				if (status === 413) throw new MinerUError(failure("FILE_TOO_LARGE", `Uploaded file is too large (${status})${diagnostic}`, false, { provider: "self-hosted-v2" }));
				if (status === 429) throw new MinerUError(failure("PROVIDER_RATE_LIMITED", `Provider rate limit exceeded (${status})${diagnostic}`, true, { provider: "self-hosted-v2" }));
				if (status >= 500) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", `MinerU server error (${status})${diagnostic}`, true, { provider: "self-hosted-v2" }));
				throw new MinerUError(failure("REMOTE_PARSE_FAILED", `MinerU returned unexpected status ${status}${diagnostic}`, false, { provider: "self-hosted-v2" }));
			}
			const contentType = response.headers.get("content-type") ?? "";
			if (!contentType.toLowerCase().includes("application/json")) throw new MinerUError(failure("REMOTE_PARSE_FAILED", `Expected application/json response, got ${contentType || "unknown"}`));
	...
	async readBoundedResponseBody(response, maxBytes, signal) {
		...
		try {
			while (true) {
				...
				if (value) {
					...
					if (totalBytes > maxBytes) throw new MinerUError(failure("RESULT_TOO_LARGE", `Response body exceeded maximum allowed size of ${maxBytes} bytes`));
					chunks.push(value);
				}
			}
		} finally {
...
async function validateJsonFile(path, maxBytes = MAX_JSON_VALIDATION_BYTES) {
	...
	try {
		...
		throw new MinerUError(failure("RESULT_ARCHIVE_INVALID", `Invalid JSON artifact: ${error instanceof Error ? error.message : String(error)}`), { cause: error });
	}
}
		...
		zip.on("entry", (entry) => {
			...
			(async () => {
				...
				try {
					...
					if (classified.some((item) => item.classification.json)) await validateJsonFile(tempPath);
...
function parseProgress(extractProgress) {
	if (typeof extractProgress === "object" && extractProgress !== null) {
		...
		if (extracted !== void 0 && total !== void 0 && total > 0) return {
			completed: extracted,
			total
		};
	}
	if (typeof extractProgress === "number" && !Number.isNaN(extractProgress)) return {
		completed: Math.round(extractProgress),
		total: 100
	};
}
var OfficialV4Provider = class {
	...
	async compatibilityKey(request, context) {
		const originAndPath = `${this.parsedBaseUrl.origin}${this.parsedBaseUrl.pathname.replace(/\/+$/, "")}`;
		return `official-v4:${createHash("sha256").update(originAndPath, "utf8").digest("hex").slice(0, 16)}:${(context.configuredVersion ?? this.config.configuredVersion ?? "v4").replace(/[^A-Za-z0-9._-]/g, "_")}:${request.semantics.model}`;
	}
	async probe(context) {
		...
		try {
			const response = await this.requestJson("GET", "/extract-results/batch/__dsh_probe__", void 0, {}, context, [
				200,
				400,
				404
			]);
			const probeCode = String(response.code).toUpperCase();
			if (probeCode === "401" || probeCode === "A0202" || probeCode === "A0211") return {
				available: false,
				provider: "official-v4",
	...
	async submit(request, sources, context) {
		...
		if (submitResponse.code !== 0) throw officialBusinessFailure(submitResponse.code, submitResponse.msg || `Submission failed with code ${String(submitResponse.code)}`, submitResponse.trace_id);
	...
	async inspect(ref, context) {
		...
		if (data.code !== 0) throw officialBusinessFailure(data.code, data.msg || `Query failed with code ${String(data.code)}`, data.trace_id);
		const extractResults = Array.isArray(data.data?.extract_result) ? data.data.extract_result : [];
		const resultsByDataId = /* @__PURE__ */ new Map();
		for (const item of extractResults) if (item.data_id && typeof item.data_id === "string") resultsByDataId.set(item.data_id, item);
		const fileSnapshots = [];
		...
		for (const file of ref.files) {
			...
			const fileFailure = fileState === "failed" ? failure("REMOTE_PARSE_FAILED", item.err_msg || "Remote document extraction failed", false, {
				provider: "official-v4",
				fileId: file.fileId,
	...
	async collect(ref, request, sink, context) {
		...
		if (data.code !== 0) throw officialBusinessFailure(data.code, data.msg || `Result query failed with code ${String(data.code)}`, data.trace_id);
		const extractResults = Array.isArray(data.data?.extract_result) ? data.data.extract_result : [];
		const resultsByDataId = /* @__PURE__ */ new Map();
		for (const item of extractResults) if (item.data_id && typeof item.data_id === "string") resultsByDataId.set(item.data_id, item);
	...
	async requestJson(method, path, bodyText, headers, context, acceptedStatuses = [200]) {
		context.signal.throwIfAborted();
		const url = `${this.parsedBaseUrl.origin}${this.parsedBaseUrl.pathname.replace(/\/+$/, "")}${path}`;
		const controller = new AbortController();
		let timedOut = false;
		...
		try {
			...
			try {
				...
				if (context.signal.aborted) throw new MinerUError(failure("CANCELLED", "Operation was cancelled", true));
				if (timedOut) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", `Request to MinerU official API timed out after ${String(context.timeoutMs)}ms`, true));
			...
			if (!acceptedStatuses.includes(status)) {
				...
				const diagnostic = parsedError ? `: ${parsedError}` : "";
				if (status === 401 || status === 403) throw new MinerUError(failure("AUTHENTICATION_FAILED", `Official MinerU authentication failed (${String(status)})${diagnostic}`, false, { provider: "official-v4" }));
				if (status === 404) throw new MinerUError(failure("JOB_NOT_FOUND", `Official MinerU resource not found (${String(status)})${diagnostic}`, false, { provider: "official-v4" }));
				if (status === 413) throw new MinerUError(failure("FILE_TOO_LARGE", `File exceeds size limit (${String(status)})${diagnostic}`, false, { provider: "official-v4" }));
				if (status === 429) throw new MinerUError(failure("PROVIDER_RATE_LIMITED", `Official MinerU rate limit exceeded (${String(status)})${diagnostic}`, true, { provider: "official-v4" }));
				if (status >= 500) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", `Official MinerU server error (${String(status)})${diagnostic}`, true, { provider: "official-v4" }));
			...
			const contentType = response.headers.get("content-type") ?? "";
			if (!contentType.toLowerCase().includes("application/json")) throw new MinerUError(failure("REMOTE_PARSE_FAILED", `Expected application/json response, got "${contentType}"`, false, { provider: "official-v4" }));
			const rawText = await this.readBoundedResponseBody(response, context.limits.maxApiResponseBytes, controller.signal);
			try {
				return JSON.parse(rawText);
			} catch (err) {
	...
	async barePutStream(uploadUrl, filePath, context) {
		context.signal.throwIfAborted();
		...
		try {
			...
			try {
				...
				if (timedOut) throw new MinerUError(failure("UPLOAD_FAILED", `Upload timed out after ${String(context.timeoutMs)}ms`, true));
			...
			if (response.status !== 200 && response.status !== 204) {
				...
				throw new MinerUError(failure("UPLOAD_FAILED", `Storage upload failed with HTTP status ${String(response.status)}${errText ? `: ${sanitizeDiagnostic(errText)}` : ""}`, true));
			}
		} finally {
			clearTimeout(timer);
			context.signal.removeEventListener("abort", onParentAbort);
	...
	async downloadZipToTemporary(cdnUrl, sink, context) {
		...
		try {
			...
			try {
				...
				if (timedOut) throw new MinerUError(failure("RESULT_DOWNLOAD_FAILED", `Download timed out after ${String(context.timeoutMs)}ms`, true));
			...
			if (response.status !== 200) throw new MinerUError(failure("RESULT_DOWNLOAD_FAILED", `Failed to download result archive, HTTP status ${String(response.status)}`, true));
			const body = response.body;
...
var ProviderRegistry = class {
	...
	async resolveForJob(job) {
		...
		if (resolved.provider.id !== job.providerId) throw new MinerUError(failure("PROVIDER_CONFIG_MISSING", "MinerU job provider identity no longer matches its configuration"));
		if (await resolved.provider.compatibilityKey(job.request, { configuredVersion: "configuredVersion" in resolved.config ? resolved.config.configuredVersion : void 0 }) !== job.providerCompatibilityKey) throw new MinerUError(failure("PROVIDER_CONFIG_MISSING", "MinerU provider configuration changed incompatibly after this job was submitted"));
		return resolved;
	}
	create(config) {
		switch (config.type) {
			case "self-hosted-v2": return new SelfHostedV2Provider(config);
			case "official-v4": return new OfficialV4Provider(config);
		}
	}
};
//#endregion
//#region src/domain/job.ts
const MINERU_JOB_SCHEMA_VERSION = 1;
...
function assertJobTransition(previous, next) {
	...
	const from = ORDER[previous];
	const to = ORDER[next];
	if (from === void 0 || to === void 0 || to < from) throw new TypeError(`invalid MinerU job transition from ${previous} to ${next}`);
}
//#endregion
...
function normalizePages(input) {
	...
	for (const token of input.split(",")) {
		...
		if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || start < 1 || end < start || end > 99999) throw new MinerUError(failure("INVALID_REQUEST", `Invalid page range token: ${trimmed}`));
		intervals.push({
			start,
			end
		});
...
function compatibleValue(modern, legacy, label) {
	if (modern !== void 0 && legacy !== void 0 && modern !== legacy) throw new MinerUError(failure("INVALID_REQUEST", `Conflicting ${label} and legacy alias`));
	return modern ?? legacy;
}
function resolvePaths(input, maxFiles) {
	if (input.file_path !== void 0 && input.file_paths !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "Use file_paths or legacy file_path, not both"));
	const paths = input.file_paths ?? (input.file_path === void 0 ? [] : [input.file_path]);
...
function resolveLegacyPages(input) {
	if (input.pages !== void 0 && (input.start_page_id !== void 0 || input.end_page_id !== void 0)) throw new MinerUError(failure("INVALID_REQUEST", "Use pages or legacy page indexes, not both"));
	if (input.pages !== void 0) return normalizePages(input.pages);
	if (input.start_page_id === void 0 && input.end_page_id === void 0) return void 0;
	const start = input.start_page_id ?? 0;
	const end = input.end_page_id ?? 99999;
	if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || start < 0 || end < start) throw new MinerUError(failure("INVALID_REQUEST", "Legacy page indexes must be non-negative and ordered"));
	if (start === 0 && end === 99999) return void 0;
...
async function prepareSource(rawPath, index, cwd, maxFileBytes, signal) {
...
async function assertSourcesUnchanged(sources, signal) {
	for (const source of sources) {
		...
		if (current.size !== expected.size || current.mtimeMs !== expected.mtimeMs || current.dev !== expected.device || current.ino !== expected.inode) throw new MinerUError(failure("INVALID_REQUEST", `${source.name} changed after hashing and before upload`, true));
	}
}
var RequestNormalizer = class {
	...
	async normalize(input, signal) {
		...
		const legacyLanguage = input.lang_list === void 0 ? void 0 : (() => {
			if (input.lang_list.length !== 1 || input.lang_list[0] === void 0) throw new MinerUError(failure("INVALID_REQUEST", "Legacy lang_list must contain exactly one language"));
		...
		const legacyModel = input.backend === void 0 ? void 0 : this.options.legacyBackendModels?.[input.backend];
		if (input.backend !== void 0 && legacyModel === void 0) throw new MinerUError(failure("UNSUPPORTED_OPTION", `Legacy backend is not mapped: ${input.backend}`));
		const model = compatibleValue(input.model, legacyModel, "model") ?? this.options.defaults.model;
		const parseMethod = input.parse_method ?? (input.ocr === void 0 ? this.options.defaults.parseMethod : input.ocr ? "ocr" : "auto");
		const ocr = parseMethod === "ocr";
		if (input.ocr !== void 0 && input.ocr !== ocr) throw new MinerUError(failure("INVALID_REQUEST", "Conflicting ocr and legacy parse_method"));
		...
		const sources = await Promise.all(paths.map((path, index) => prepareSource(path, index, this.options.cwd, this.options.maxFileBytes, signal)));
		return {
			sources,
			request: {
				schemaVersion: 1,
...
function providerRef(job) {
	return job.resolution.kind === "cache-hit" ? void 0 : job.resolution.ref;
}
function withProviderRef(resolution, ref) {
	if (resolution.kind === "shared-operation") return {
		...resolution,
		ref
	};
	if (resolution.kind === "provider") return {
		kind: "provider",
		ref
	};
	return resolution;
...
function fileViews(job) {
	return job.files.map((file) => ({
		file_id: file.fileId,
		name: file.name,
		state: file.state,
		...file.progress === void 0 ? {} : { progress: file.progress },
		...file.failure === void 0 ? {} : { failure: file.failure }
	}));
}
function statusView(job) {
	return {
		...
		files: fileViews(job),
		result_available: job.resultId !== void 0 && (job.state === "completed" || job.state === "partially-completed"),
		created_at: job.createdAt,
		updated_at: job.updatedAt,
		...job.failure === void 0 ? {} : { failure: job.failure }
	};
}
function submitView(job) {
	...
	return {
		...
		...status.failure === void 0 ? {} : { failure: status.failure }
	};
}
var MinerUService = class {
	...
	async callContext(config, signal) {
		signal.throwIfAborted();
		...
		signal.throwIfAborted();
		if (config.type === "official-v4" && credential === void 0) throw new MinerUError(failure("CREDENTIAL_MISSING", `Credential ${config.apiKeyEnv} is not configured`));
	...
	legacyBackendModels(config) {
		if (config.type === "self-hosted-v2") return Object.fromEntries(Object.entries(config.modelMap).map(([model, backend]) => [backend, model]));
		return {
			pipeline: "pipeline",
			"vlm-engine": "vlm",
			"hybrid-engine": "vlm",
	...
	async submit(session, input, signal) {
		...
		if (prepared.request.files.length !== 1) throw new MinerUError(failure("INVALID_REQUEST", "This release accepts exactly one file per model tool submission"));
		validateProviderCapabilities(prepared.request, active.provider.capabilities);
		const compatibility = await active.provider.compatibilityKey(prepared.request, { configuredVersion: "configuredVersion" in active.config ? active.config.configuredVersion : void 0 });
		...
		let job = this.newJob(sessionId, prepared, active, compatibility, cacheKey, { kind: "provider" }, void 0, "queued");
		await this.options.jobs.create(session, job);
		const acquired = this.options.operations.acquire(cacheKey, current.polling.operationTimeoutMs, (operation) => this.runOperation(operation, prepared, active, compatibility));
		if (!acquired.created) job = await this.options.jobs.update(session, job.id, (record) => ({
			...record,
			resolution: {
	...
	newJob(sessionId, prepared, active, compatibility, cacheKey, resolution, resultId, state) {
		const now = Date.now();
		const file = prepared.request.files[0];
		...
		return {
			...
			sourceFiles: prepared.request.files.map((source) => ({
	...
	async syncSubmission(session, jobId, submitted) {
		return this.options.jobs.update(session, jobId, (current) => {
			...
			return {
				...
				resolution: ref === void 0 ? current.resolution : withProviderRef(current.resolution, ref),
	...
	async updateWaiters(operation, mutate) {
		await Promise.all([...operation.waiters.values()].map(async (waiter) => {
			try {
				await this.options.jobs.update(waiter.session, waiter.jobId, mutate);
			} catch (error) {
				if ((error instanceof MinerUError ? error.failure.code : void 0) !== "JOB_NOT_FOUND") throw error;
			}
		}));
	}
	snapshotFiles(current, snapshot) {
		const byId = new Map(snapshot.files.map((file) => [file.fileId, file]));
		return current.files.map((file) => {
			const next = byId.get(file.fileId);
			if (next === void 0) return file;
			const { failure: _oldFailure, progress: _oldProgress, ...stable } = file;
			return {
				...stable,
				state: next.state === "queued" ? "processing" : next.state,
				...next.progress === void 0 ? {} : { progress: next.progress },
				...next.failure === void 0 ? {} : { failure: next.failure }
			};
		});
	}
	async runOperation(operation, prepared, resolved, compatibility, recoveryRef) {
		let ref = recoveryRef;
		try {
			...
			if (ref === void 0) {
				...
				const submission = await resolved.provider.submit(prepared.request, prepared.sources, await this.callContext(resolved.config, operation.controller.signal));
				ref = submission.ref;
				snapshot = {
					state: submission.state,
					files: submission.files
				};
			} else snapshot = await resolved.provider.inspect(ref, await this.callContext(resolved.config, operation.controller.signal));
			await this.updateWaiters(operation, (current) => isTerminalJobState(current.state) ? current : {
				...current,
			...
			const transaction = this.options.results.beginTransaction(operation.id, prepared.request, {
				providerId: resolved.provider.id,
				providerConfigId: resolved.config.id,
				compatibilityKey: compatibility
			}, operation.controller.signal);
			const collection = await resolved.provider.collect(ref, prepared.request, transaction, await this.callContext(resolved.config, operation.controller.signal));
			const file = prepared.request.files[0];
			...
			const normalized = ref === void 0 && operation.controller.signal.aborted ? failure("INTERRUPTED_UPLOAD", "Upload was interrupted before a recoverable provider reference was stored", true) : toMinerUFailure(error);
			await this.updateWaiters(operation, (current) => {
				if (isTerminalJobState(current.state)) return current;
				if (ref !== void 0 && normalized.retryable) return {
					...current,
					state: "processing",
					resolution: withProviderRef(current.resolution, ref),
					failure: normalized
	...
	async status(session, jobId, signal) {
		...
		const acquired = this.options.operations.acquire(job.cacheKey, this.config().polling.operationTimeoutMs, (operation) => this.runOperation(operation, prepared, resolved, job.providerCompatibilityKey, ref));
		acquired.operation.attach({
			jobId: job.id,
			sessionId: job.sessionId,
			session
		...
		try {
			await acquired.operation.waitForSubmission(signal);
		} catch (error) {
			if (signal.aborted) throw error;
		}
		job = await this.options.jobs.require(session, job.id);
	...
	async result(session, jobId, signal) {
		const status = await this.status(session, jobId, signal);
		if (status.state !== "completed" && status.state !== "partially-completed") throw new MinerUError(failure("RESULT_NOT_READY", `MinerU job ${jobId} is ${status.state}`, true));
		const job = await this.options.jobs.require(session, jobId);
		if (job.resultId === void 0) throw new MinerUError(failure("CACHE_EVICTED", "Completed job no longer has a result reference"));
		const manifest = await this.options.results.get(job.cacheKey, job.request.requiredArtifacts, signal);
		if (manifest === void 0 || manifest.id !== job.resultId) throw new MinerUError(failure("CACHE_EVICTED", "Published MinerU result is missing or corrupt"));
		return this.projectResult(job, manifest);
	}
	async projectResult(job, manifest) {
		...
		const preview = markdown === void 0 ? void 0 : await this.markdownPreview(this.options.results.resolveArtifactAbsolutePath(job.cacheKey, markdown.relativePath), markdown.bytes, limit);
		...
		const view = {
			...
			files: [{
				...
				name: job.sourceFiles[0]?.name ?? document.name,
				artifacts
			}],
	...
	async parseDocument(session, input, signal, pollTimeoutMs) {
		...
		if (operation === void 0) return this.status(session, job.id, signal);
		...
		try {
			...
			if (waitController.signal.aborted && !isTerminalJobState(current.state)) return {
				...statusView(current),
				failure: failure("POLL_TIMEOUT", "Synchronous wait timed out; query this job_id later", true),
				poll_timed_out: true
			};
		} finally {
			clearTimeout(timer);
		...
		const current = await this.options.jobs.require(session, job.id);
		return current.state === "completed" || current.state === "partially-completed" ? this.result(session, job.id, signal) : statusView(current);
	}
};
//#endregion
...
var SharedOperation = class {
	...
	constructor(cacheKey) {
		...
		this.submission.promise.catch(() => void 0);
...
var SharedOperationRegistry = class {
	...
	disposed = false;
	acquire(cacheKey, timeoutMs, runner) {
		...
		timeout.unref?.();
		Promise.resolve().then(() => runner(operation)).then((outcome) => operation.resolve(outcome), (error) => operation.reject(error)).finally(() => {
...
*/
const VALID_JOB_STATES = /* @__PURE__ */ new Set([
	"queued",
	"uploading",
	"processing",
	"collecting",
...
const VALID_FILE_STATES = /* @__PURE__ */ new Set([
	"queued",
	"uploading",
	"processing",
	"completed",
...
const VALID_ERROR_CODES = /* @__PURE__ */ new Set([
	"INVALID_REQUEST",
	"FILE_NOT_FOUND",
	"FILE_TOO_LARGE",
	"UNSUPPORTED_OPTION",
...
function parseParseSemantics(input) {
	...
	if (obj["pages"] !== void 0) {
		...
		if (!PAGE_RANGES.test(rawPages)) throw new TypeError(`invalid ParseSemantics.pages format: "${rawPages}"`);
		pages = rawPages;
	}
	return {
		model,
...
function parseProviderSubmittedFile(input) {
	const obj = assertPlainObject(input, "ProviderSubmittedFile");
	assertNoAdditionalProperties(obj, [
		"dataId",
		"fileId",
		"name"
	], "ProviderSubmittedFile");
	return {
		dataId: assertNoUrlOrSecret(assertNonEmptyString(obj["dataId"], "ProviderSubmittedFile.dataId"), "ProviderSubmittedFile.dataId"),
		fileId: asFileId(assertNonEmptyString(obj["fileId"], "ProviderSubmittedFile.fileId")),
		name: assertSafeFileName(obj["name"], "ProviderSubmittedFile.name")
	};
}
function parseProviderJobRef(input) {
	...
	if (provider === "self-hosted-v2") {
		assertNoAdditionalProperties(obj, [
			...
			"taskId",
			"files"
		], "ProviderJobRef (self-hosted-v2)");
		const taskId = assertNoUrlOrSecret(assertNonEmptyString(obj["taskId"], "ProviderJobRef.taskId"), "ProviderJobRef.taskId");
		if (!Array.isArray(obj["files"])) throw new TypeError("ProviderJobRef.files must be an array");
		return {
			provider: "self-hosted-v2",
			taskId,
			files: obj["files"].map((f) => parseProviderSubmittedFile(f))
		};
	}
	if (provider === "official-v4") {
		assertNoAdditionalProperties(obj, [
			...
			"batchId",
			"files"
		], "ProviderJobRef (official-v4)");
		const batchId = assertNoUrlOrSecret(assertNonEmptyString(obj["batchId"], "ProviderJobRef.batchId"), "ProviderJobRef.batchId");
		if (!Array.isArray(obj["files"])) throw new TypeError("ProviderJobRef.files must be an array");
		return {
			provider: "official-v4",
			batchId,
...
function parseJobResolution(input) {
	...
	if (kind === "shared-operation") {
		assertNoAdditionalProperties(obj, [
			...
			"operationId",
			"ref"
		], "JobResolution (shared-operation)");
		const operationId = asOperationId(assertNonEmptyString(obj["operationId"], "JobResolution.operationId"));
		const ref = obj["ref"] === void 0 ? void 0 : parseProviderJobRef(obj["ref"]);
		return ref === void 0 ? {
			kind: "shared-operation",
			operationId
		} : {
	...
	if (kind === "provider") {
		...
		const ref = obj["ref"] === void 0 ? void 0 : parseProviderJobRef(obj["ref"]);
		return ref === void 0 ? { kind: "provider" } : {
			kind: "provider",
			ref
		};
	}
...
function parseJobSourceFile(input) {
	...
	return {
		fileId: asFileId(assertNonEmptyString(obj["fileId"], "JobSourceFile.fileId")),
		name: assertSafeFileName(obj["name"], "JobSourceFile.name"),
		bytes: assertNonNegativeSafeInteger(obj["bytes"], "JobSourceFile.bytes"),
		sha256: assertSha256(obj["sha256"], "JobSourceFile.sha256")
	};
}
function parseMinerUFailure(input) {
	...
	const code = obj["code"];
	if (typeof code !== "string" || !VALID_ERROR_CODES.has(code)) throw new TypeError(`invalid MinerUFailure.code: "${String(code)}"`);
	const message = assertNonEmptyString(obj["message"], "MinerUFailure.message");
	const retryable = obj["retryable"];
	if (typeof retryable !== "boolean") throw new TypeError("MinerUFailure.retryable must be a boolean");
	let provider;
	if (obj["provider"] !== void 0) {
		const rawProvider = obj["provider"];
		if (typeof rawProvider !== "string" || !VALID_PROVIDERS.has(rawProvider)) throw new TypeError(`invalid MinerUFailure.provider: "${String(rawProvider)}"`);
		provider = rawProvider;
	}
	const providerCode = obj["providerCode"] === void 0 ? void 0 : assertNoUrlOrSecret(assertNonEmptyString(obj["providerCode"], "MinerUFailure.providerCode"), "MinerUFailure.providerCode");
	const traceId = obj["traceId"] === void 0 ? void 0 : assertNoUrlOrSecret(assertNonEmptyString(obj["traceId"], "MinerUFailure.traceId"), "MinerUFailure.traceId");
	const fileId = obj["fileId"] === void 0 ? void 0 : asFileId(assertNonEmptyString(obj["fileId"], "MinerUFailure.fileId"));
	return {
		code,
		message,
		retryable,
...
function parseMinerUFileStatus(input) {
	...
	if (typeof state !== "string" || !VALID_FILE_STATES.has(state)) throw new TypeError(`invalid MinerUFileStatus.state: "${String(state)}"`);
	const resultId = obj["resultId"] === void 0 ? void 0 : asResultId(assertNonEmptyString(obj["resultId"], "MinerUFileStatus.resultId"));
	const failure = obj["failure"] === void 0 ? void 0 : parseMinerUFailure(obj["failure"]);
	let progress;
	if (obj["progress"] !== void 0) {
		...
		progress = {
			completed: assertNonNegativeSafeInteger(progObj["completed"], "MinerUFileStatus.progress.completed"),
			total: assertNonNegativeSafeInteger(progObj["total"], "MinerUFileStatus.progress.total")
		};
	}
	return {
		fileId,
...
function parseMinerUJobRecord(input) {
	...
	if (obj["schemaVersion"] !== 1) throw new TypeError(`invalid MinerUJobRecord schemaVersion: expected 1, got ${String(obj["schemaVersion"])}`);
	...
	if (typeof providerId !== "string" || !VALID_PROVIDERS.has(providerId)) throw new TypeError(`invalid MinerUJobRecord.providerId: "${String(providerId)}"`);
	const providerConfigId = asProviderConfigId(assertNonEmptyString(obj["providerConfigId"], "MinerUJobRecord.providerConfigId"));
	const providerCompatibilityKey = assertNoUrlOrSecret(assertNonEmptyString(obj["providerCompatibilityKey"], "MinerUJobRecord.providerCompatibilityKey"), "MinerUJobRecord.providerCompatibilityKey");
	if (!Array.isArray(obj["sourceFiles"]) || obj["sourceFiles"].length === 0) throw new TypeError("MinerUJobRecord.sourceFiles must be a non-empty array");
	...
	const state = obj["state"];
	if (typeof state !== "string" || !VALID_JOB_STATES.has(state)) throw new TypeError(`invalid MinerUJobRecord.state: "${String(state)}"`);
	const resolution = parseJobResolution(obj["resolution"]);
	if (!Array.isArray(obj["files"]) || obj["files"].length === 0) throw new TypeError("MinerUJobRecord.files must be a non-empty array");
	const files = obj["files"].map((f) => parseMinerUFileStatus(f));
	const resultId = obj["resultId"] === void 0 ? void 0 : asResultId(assertNonEmptyString(obj["resultId"], "MinerUJobRecord.resultId"));
	const failure = obj["failure"] === void 0 ? void 0 : parseMinerUFailure(obj["failure"]);
...
var ProcessLock = class {
	...
	async acquire(signal) {
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			signal?.throwIfAborted();
			try {
				await writeFile(this.lockFilePath, data, {
					flag: "wx",
					mode: 384
				});
				this.acquired = true;
				...
				try {
					...
					throwMinerU("STORAGE_LOCKED", `MinerU storage root "${this.paths.root}" has an invalid lock file; refusing unsafe recovery`);
				}
				if (isProcessAlive(existing.pid)) {
					...
					throwMinerU("STORAGE_LOCKED", `MinerU storage root "${this.paths.root}" is locked by active process PID ${String(existing.pid)} (host: ${existing.hostname})`);
				}
				try {
		...
		try {
			...
			if (finalErr.code === "EEXIST") throwMinerU("STORAGE_LOCKED", `Failed to acquire lock on "${this.paths.root}"`);
			throw finalErr;
		}
	}
	async release() {
...
//#endregion
//#region src/storage/job-repository.ts
/**
...
function extractSessionId(session) {
	if (session !== null && typeof session === "object" && "header" in session && session.header !== null && typeof session.header === "object" && "id" in session.header) return asSessionId(String(session.header.id));
...
var JobRepository = class {
	...
	async create(session, job) {
		const sessionId = extractSessionId(session);
		if (job.sessionId !== sessionId) throwMinerU("JOB_ACCESS_DENIED", `Job sessionId "${job.sessionId}" does not match caller session "${sessionId}"`);
		const validated = parseMinerUJobRecord(job);
		await this.atomicWrite(sessionId, validated.id, validated);
		return validated;
	}
	async get(session, jobId) {
		const sessionId = extractSessionId(session);
		...
		if (record.sessionId !== sessionId) throwMinerU("JOB_ACCESS_DENIED", `Job "${validJobId}" does not belong to session "${sessionId}"`);
		return record;
	}
	async require(session, jobId) {
		const record = await this.get(session, jobId);
		if (record === void 0) throwMinerU("JOB_NOT_FOUND", `MinerU job "${asJobId(jobId)}" not found for session "${extractSessionId(session)}"`);
		return record;
	}
	async update(session, jobId, mutator) {
		const sessionId = extractSessionId(session);
		const validJobId = asJobId(jobId);
		...
		return this.mutex.lock(mutexKey, async () => {
			...
			const validated = parseMinerUJobRecord({
				...next,
	...
	async list(session) {
		...
		for (const entry of entries) {
			if (!entry.startsWith("mj_") || !entry.endsWith(".json") || entry.includes(".tmp.")) continue;
	...
	async atomicWrite(sessionId, jobId, record) {
		const sessionDir = this.paths.jobDir(sessionId);
		...
		try {
			const handle = await open(tempFile, "wx", 384);
			try {
...
async function setReadOnlyRecursive(dirPath) {
	try {
		...
		for (const item of items) {
			...
			} else if (item.isFile()) await chmod(full, 292).catch(() => void 0);
		}
		await chmod(dirPath, 365).catch(() => void 0);
	} catch {}
}
...
var ResultRepository = class {
	...
	async verifyArtifact(path, artifact, signal) {
		signal?.throwIfAborted();
	...
	async verifyManifestArtifacts(manifest, resolvePath, signal) {
		for (const artifact of manifest.files[0].artifacts) await this.verifyArtifact(resolvePath(artifact.relativePath), artifact, signal);
	}
	async commitTransaction(tx, manifest, signal) {
		...
		try {
			await this.verifyManifestArtifacts(validated, (relativePath) => this.paths.resolveStagingArtifactPath(tx.operationId, relativePath), signal);
		} catch (error) {
			await tx.abort();
		...
		await writeFile(stagingManifestPath, JSON.stringify(validated, null, 2), {
			encoding: "utf8",
			mode: 384
		});
	...
	async quarantine(sourcePath, reason = "quarantine") {
		...
		await chmod(sourcePath, 493).catch(() => void 0);
		try {
...
const failureSchema = {
	...
	properties: {
		code: {
			type: "string",
			description: "Unified error code."
		},
		message: {
			type: "string",
		...
		providerCode: {
			type: "string",
			description: "Upstream provider error code."
		},
		traceId: {
			type: "string",
			description: "Upstream trace or request ID."
		},
		fileId: {
			type: "string",
			description: "Affected file identifier."
		}
	},
	additionalProperties: false
};
const fileStatusSchema = {
	type: "object",
	description: "Individual file processing status.",
	properties: {
		file_id: {
			type: "string",
			description: "Stable internal file identifier."
		},
		name: {
			type: "string",
		...
		state: {
			type: "string",
			description: "File lifecycle state."
		},
		progress: {
			type: "object",
...
const artifactViewSchema = {
	...
	properties: {
		kind: {
			type: "string",
			description: "Artifact kind (markdown, layout, model-output, content-list, images)."
		},
		path: {
			type: "string",
			description: "Absolute local filesystem path to the artifact."
		},
		bytes: {
			type: "integer",
...
const resultFileViewSchema = {
	...
	properties: {
		...
		artifacts_truncated: {
			type: "boolean",
			description: "Whether artifact list was truncated to fit limit."
		}
	},
	additionalProperties: false
};
...
function clampRenderText(rendered, limit) {
	if (limit === void 0 || limit <= 0 || rendered.length <= limit) return rendered;
	const suffix = "\n\n[Output truncated to limit]";
	const sliceLen = Math.max(0, limit - 29);
...
function renderSubmit(value) {
	...
	if (value.files.length > 0) {
		lines.push("- Files:");
		for (const file of value.files) lines.push(`  - ${file.name} (${file.state})`);
	}
	if (value.failure !== void 0) lines.push(`- Failure: [${value.failure.code}] ${value.failure.message}`);
	return [{
		type: "text",
		text: lines.join("\n")
	}];
...
function renderStatus(value) {
	...
	if (value.files.length > 0) {
		...
		for (const file of value.files) {
			const progress = file.progress !== void 0 ? ` (progress: ${String(file.progress.completed)}/${String(file.progress.total)})` : "";
			lines.push(`  - ${file.name}: ${file.state}${progress}`);
		}
	}
	if (value.failure !== void 0) lines.push(`- Error: [${value.failure.code}] ${value.failure.message}`);
	return [{
		type: "text",
		text: lines.join("\n")
	}];
...
function renderParseDocument(value) {
	if ("manifest_path" in value && (value.state === "completed" || value.state === "partially-completed")) return renderResult(value);
	return renderStatus(value);
}
function requireAgentSession(exec) {
	const agent = exec.agent;
	if (agent === void 0) throw new Error("MinerU operations require an authenticated agent session (UNAUTHENTICATED_SESSION)");
	return {
		session: agent.session,
		signal: exec.signal
	};
}
function registerTools(ctx, getService, _getConfig) {
	const disposers = [];
		...
		output: {
			schema: {
				...
				properties: {
					available: {
						type: "boolean",
						description: "Whether the provider service is available."
					},
					provider: {
						type: "string",
						enum: ["self-hosted-v2", "official-v4"],
					...
					protocol_version: {
						type: "string",
						description: "Provider protocol version."
					},
					server_version: {
						type: "string",
						description: "Upstream server version string."
					},
					queue: {
						type: "object",
						...
						properties: {
							...
							completed: {
								type: "integer",
								description: "Completed tasks retained by upstream."
							},
							failed: {
								type: "integer",
					...
					diagnostics: {
						type: "string",
						description: "Diagnostic notes or connection error information."
					}
				},
				additionalProperties: false
			},
		...
		output: {
			schema: {
				...
				properties: {
					job_id: {
						type: "string",
						description: "Unique MinerU job ID (mj_...)."
					},
					state: {
						type: "string",
						enum: [
					...
					result_available: {
						type: "boolean",
						description: "Whether result is already available (e.g. cache hit)."
					},
					failure: failureSchema
				},
				additionalProperties: false
			},
			render: (_args, value) => renderSubmit(value)
		},
		execute: async (args, exec) => {
			const { session, signal } = requireAgentSession(exec);
			return await getService().submit(session, args, signal);
		}
	})));
	disposers.push(ctx.tools.register(defineTool({
		...
		output: {
			schema: {
				...
				properties: {
					...
					result_available: {
						type: "boolean",
						description: "Whether parsing result is ready to fetch."
					},
					failure: failureSchema,
					created_at: {
						type: "number",
		...
		execute: async (args, exec) => {
			...
			if (typeof jobId !== "string" || jobId.trim() === "") throw new Error("job_id is required");
		...
		output: {
			schema: {
				...
				properties: {
					...
					cache_hit: {
						type: "boolean",
						description: "Whether the result was served from global cache."
					},
					result_id: {
						type: "string",
						description: "Global content-addressed result ID."
					},
					files: {
						type: "array",
					...
					markdown_preview: {
						type: "string",
						description: "Inline markdown preview text."
					},
					preview_truncated: {
						type: "boolean",
						description: "Whether inline preview was truncated."
					},
					manifest_path: {
						type: "string",
						description: "Local path to the result manifest.json file."
					},
					output_limit_chars: {
						type: "integer",
						description: "Configured maximum inline character limit."
					}
				},
				additionalProperties: false
			},
		...
		output: {
			schema: {
				...
				properties: {
					...
					result_id: {
						type: "string",
						description: "Result manifest ID if completed."
					},
					files: {
						type: "array",
						...
						items: {
							...
							properties: {
								...
								state: {
									type: "string",
									description: "File state if in status view."
								},
								progress: {
									type: "object",
									properties: {
					...
					preview_truncated: {
						type: "boolean",
						description: "Whether markdown preview was truncated."
					},
					manifest_path: {
						type: "string",
						description: "Absolute path to result manifest.json."
					},
					output_limit_chars: {
						type: "integer",
						description: "Inline output character limit."
					},
					result_available: {
						type: "boolean",
						description: "Whether result is available."
					},
					failure: failureSchema,
					poll_timed_out: {
						type: "boolean",
						description: "Whether synchronous poll timed out."
					},
					created_at: {
						type: "number",
						description: "Creation timestamp in epoch milliseconds."
...
function registerRpc(ctx, deps) {
	...
	const dispose = ctx.connection.rpc.handle(RPC_CHANNEL, async (endpoint, payload, signal) => {
		try {
			switch (endpoint) {
				case "mineru/config.get": return ok({ config: deps.getConfig() });
				case "mineru/config.set": {
					if (payload === void 0 || payload === null || typeof payload !== "object") return fail("payload must be an object with config property", "invalid-argument");
					const validated = migrateConfig(payload.config);
					return ok({ config: await deps.setConfig(validated) });
				}
				case "mineru/probe": {
					const draft = payload?.provider;
					return ok(await deps.probe(draft, signal));
				}
...
const inject = ["tools"];
const ProviderSchema = import_lib.default.union([import_lib.default.object({
	id: import_lib.default.string(),
	type: import_lib.default.string(),
	baseURL: import_lib.default.string(),
	apiKeyEnv: import_lib.default.string().role("credential-ref"),
	modelMap: import_lib.default.object({
		pipeline: import_lib.default.string(),
		vlm: import_lib.default.string()
	}),
	configuredVersion: import_lib.default.string(),
	allowInsecureHttp: import_lib.default.boolean()
}), import_lib.default.object({
	id: import_lib.default.string(),
	type: import_lib.default.string(),
	baseURL: import_lib.default.string(),
	apiKeyEnv: import_lib.default.string().role("credential-ref"),
	models: import_lib.default.array(import_lib.default.union(["pipeline", "vlm"])),
	configuredVersion: import_lib.default.string()
})]);
/** Entry schema accepts both the provider config and the legacy flat config. */
const Config = import_lib.default.object({
	schemaVersion: import_lib.default.number(),
	activeProvider: import_lib.default.string(),
	providers: import_lib.default.array(ProviderSchema),
	defaults: import_lib.default.object({
		model: import_lib.default.union(["pipeline", "vlm"]),
		ocr: import_lib.default.boolean(),
		parseMethod: import_lib.default.union([
			"auto",
			"txt",
			"ocr"
		]),
		language: import_lib.default.string(),
		formula: import_lib.default.boolean(),
		table: import_lib.default.boolean(),
		artifacts: import_lib.default.array(import_lib.default.union([
			"markdown",
			"layout",
			"model-output",
			"content-list",
	...
	storage: import_lib.default.object({
		storageRoot: import_lib.default.string(),
		cacheEnabled: import_lib.default.boolean(),
		retainSources: import_lib.default.boolean(),
		stagingTtlMs: import_lib.default.number()
	}),
	polling: import_lib.default.object({
		pollIntervalMs: import_lib.default.number(),
		pollTimeoutMs: import_lib.default.number(),
		requestTimeoutMs: import_lib.default.number(),
		operationTimeoutMs: import_lib.default.number()
	}),
	output: import_lib.default.object({ maxInlineChars: import_lib.default.number() }),
	limits: import_lib.default.object({
		maxFilesPerRequest: import_lib.default.number(),
		maxFileBytes: import_lib.default.number(),
		maxApiResponseBytes: import_lib.default.number(),
		maxZipDownloadBytes: import_lib.default.number(),
		maxZipEntries: import_lib.default.number(),
		maxZipEntryBytes: import_lib.default.number(),
		maxZipTotalBytes: import_lib.default.number(),
		maxZipCompressionRatio: import_lib.default.number()
	}),
	baseURL: import_lib.default.string(),
	apiKeyEnv: import_lib.default.string().role("credential-ref"),
	defaultBackend: import_lib.default.string(),
	defaultParseMethod: import_lib.default.string(),
	defaultLang: import_lib.default.string(),
	pollIntervalMs: import_lib.default.number(),
	pollTimeoutMs: import_lib.default.number(),
	requestTimeoutMs: import_lib.default.number(),
	maxMdOutputChars: import_lib.default.number()
});
...
async function apply(ctx, entryConfig = {}) {
	...
	const runtimeConfig = () => ({
		...persistedConfig,
		storage: {
			...persistedConfig.storage,
			storageRoot: fixedStorageRoot
		}
	});
	...
	try {
		...
		ctx.inject(["settings"], (settingsCtx) => {
			const settings = settingsCtx.get("settings") ?? ctx.get("settings");
			if (settings !== void 0) {
				...
				return settingsScope.watch((next) => {
					persistedConfig = migrateConfig(next);
				});
			}
		});
		ctx.inject(["connection"], (connectionCtx) => {
			return registerRpc(connectionCtx, {
				...
				setConfig: async (value) => {
					...
					if (settingsScope !== void 0) await settingsScope.replace(asObject(next));
