...
import { link, lstat, mkdir, mkdtemp, open, opendir, readFile, readdir, realpath, rename, rm, stat, unlink, writeFile } from "node:fs/promises";
...
const MINERU_CONFIG_SCHEMA_VERSION = 2;
const MIN_INLINE_IMAGE_BUDGET = 0;
const MAX_INLINE_IMAGE_BUDGET = 100;
...
const ALLOWED_OUTPUT_KEYS = /* @__PURE__ */ new Set(["maxInlineChars", "maxInlineImages"]);
...
const LEGACY_ARTIFACT_KINDS = /* @__PURE__ */ new Set([
	"markdown",
	"layout",
	"model-output",
	"content-list",
...
function boundedInteger$1(value, fallback, label, min, max) {
	const result = value === void 0 ? fallback : value;
	if (typeof result !== "number" || !Number.isSafeInteger(result) || result < min || result > max) throw new TypeError(`${label} must be an integer between ${String(min)} and ${String(max)}`);
	return result;
}
...
function hasLegacyV1Fields(input) {
...
function migrateV1Fields(input) {
	const defaults = record(input.defaults ?? {}, "defaults");
	const legacyArtifacts = defaults.artifacts;
	if (legacyArtifacts !== void 0 && (!Array.isArray(legacyArtifacts) || legacyArtifacts.some((item) => typeof item !== "string" || !LEGACY_ARTIFACT_KINDS.has(item)))) throw new TypeError("defaults.artifacts contains an unsupported artifact");
	const migratedDefaults = { ...defaults };
	delete migratedDefaults.artifacts;
	const limits = record(input.limits ?? {}, "limits");
	if (limits.maxFilesPerRequest !== void 0) positive(limits.maxFilesPerRequest, 1, "limits.maxFilesPerRequest");
...
function parseCanonical(input, fallback) {
	if (input.schemaVersion !== void 0) {
		if (input.schemaVersion !== 2) throw new TypeError(`unsupported schemaVersion: ${String(input.schemaVersion)}`);
	}
	...
	const result = {
		...
		output: {
			maxInlineChars: boundedPositive(output.maxInlineChars, fallback.output.maxInlineChars, "output.maxInlineChars", 1024, 1e6),
			maxInlineImages: boundedInteger$1(output.maxInlineImages, fallback.output.maxInlineImages, "output.maxInlineImages", 0, 100)
		},
		limits: {
...
function parseConfigInput(value, allowCurrentLegacyFields) {
	...
	if (input.schemaVersion === 1) return {
		config: parseCanonical(migrateV1Fields(input), fallback),
		migrated: true,
		migratedFrom: 1
	};
	if (allowCurrentLegacyFields && (input.schemaVersion === void 0 || input.schemaVersion === 2) && hasLegacyV1Fields(input)) return {
		config: parseCanonical(migrateV1Fields(input), fallback),
		migrated: true
	};
	return {
		config: parseCanonical(input, fallback),
		migrated: false
	};
}
/** Parse startup/settings input, including legacy fields merged over a current composition base. */
function parseConfigWithMigration(value) {
...
function pruneObjectSection(nextSection, baseSection) {
	const sectionDiff = {};
	const allKeys = /* @__PURE__ */ new Set([...Object.keys(nextSection), ...Object.keys(baseSection)]);
	for (const key of allKeys) if (!deepEqualJson(nextSection[key], baseSection[key])) sectionDiff[key] = nextSection[key];
	return Object.keys(sectionDiff).length > 0 ? sectionDiff : void 0;
}
function pruneConfigToDiff(next, base = defaultMinerUConfig()) {
	const diff = {};
	if (next.activeProvider !== base.activeProvider) diff.activeProvider = next.activeProvider;
	if (!deepEqualJson(next.providers, base.providers)) diff.providers = next.providers;
	if (!deepEqualJson(next.defaults, base.defaults)) diff.defaults = next.defaults;
	for (const section of [
		...
		const sectionDiff = pruneObjectSection(next[section], base[section]);
		if (sectionDiff !== void 0) diff[section] = sectionDiff;
	}
...
function detectBloatedSettingsOps(userSection, base = defaultMinerUConfig()) {
	if (typeof userSection !== "object" || userSection === null || Array.isArray(userSection)) return [];
	const ops = [];
	for (const section of [
		"limits",
		"polling",
		...
		if (typeof userVal === "object" && userVal !== null && !Array.isArray(userVal)) {
			if (deepEqualJson(userVal, base[section])) ops.push({
				op: "unset",
				path: [section]
			});
			else {
				const userObj = userVal;
				const baseObj = base[section];
				for (const key of Object.keys(userObj)) if (deepEqualJson(userObj[key], baseObj[key])) ops.push({
					op: "unset",
					path: [section, key]
				});
			}
	...
	if (deepEqualJson(outputVal, base.output)) ops.push({
		op: "unset",
		path: ["output"]
	});
	else if (typeof outputVal === "object" && outputVal !== null && !Array.isArray(outputVal)) {
		const outputObj = outputVal;
		const baseOutputObj = base.output;
		for (const key of Object.keys(outputObj)) if (deepEqualJson(outputObj[key], baseOutputObj[key])) ops.push({
			op: "unset",
			path: ["output", key]
		});
	}
	if (deepEqualJson(userSection.defaults, base.defaults)) ops.push({
		op: "unset",
		path: ["defaults"]
	});
	if (deepEqualJson(userSection.providers, base.providers)) ops.push({
		op: "unset",
		path: ["providers"]
	});
	if (Object.hasOwn(userSection, "activeProvider") && userSection.activeProvider === base.activeProvider) ops.push({
		op: "unset",
		path: ["activeProvider"]
	});
	const remainingKeys = new Set(Object.keys(userSection).filter((k) => {
		if (k === "schemaVersion") return false;
		const val = userSection[k];
		if (typeof val === "object" && val !== null && !Array.isArray(val) && Object.keys(val).length === 0) return false;
		return true;
	}));
	for (const op of ops) if (op.path.length === 1) remainingKeys.delete(op.path[0]);
		...
		if (typeof parentVal === "object" && parentVal !== null && !Array.isArray(parentVal)) {
			const parentKeys = Object.keys(parentVal);
			const unsetKeys = ops.filter((o) => o.path.length === 2 && o.path[0] === parentKey).map((o) => o.path[1]);
			if (parentKeys.every((k) => unsetKeys.includes(k))) remainingKeys.delete(parentKey);
		}
	}
	if (remainingKeys.size === 0 && Object.hasOwn(userSection, "schemaVersion")) ops.push({
		op: "unset",
		path: ["schemaVersion"]
	});
	return ops;
...
function formatTocMarkdown(headings, options, ranges) {
	...
	for (const heading of headings) {
		...
		if (heading.block_id && ranges) {
			...
			const coordinates = {
				...
				...heading.page === void 0 ? {} : { page: heading.page }
			};
			ranges.push({
				...coordinates,
				start: nextOffset,
				locator_end: nextOffset,
				end: markerStart
			});
			ranges.push({
				...coordinates,
...
function extractBlocksMarkdown(contentList, pagesSet, focusSet, imageArtifacts) {
	...
	const append = (text) => {
		renderedLength += text.length + (renderedBlocks.length ? 2 : 0);
	...
	for (const block of contentList) {
		...
		if (block.block_id !== void 0) append(`[${block.block_id}${pageNum === void 0 ? "" : " · Page " + pageNum}${block.document_label ? " · " + block.document_label : ""}]`);
		...
		if (cat === "image") {
			...
			const pageLabel = pageNum === void 0 ? "" : " (Page " + String(pageNum) + ")";
			let md = "> " + imageLabel + pageLabel + (mediaType === void 0 || imgPath === "" ? " unavailable" : "") + (caption ? ": " + caption : "");
			...
			if (lower === "code") {
				...
				if (level !== void 0 && !text.startsWith("#")) append(`${"#".repeat(level)} ${text}`);
				else if (text) append(text);
			}
		}
		if (block.block_id) ranges.push({
...
async function readMarkdownFile(path, totalBytes, signal) {
	signal?.throwIfAborted();
	...
	try {
		...
		return {
			text: new TextDecoder$1("utf-8", { fatal: true }).decode(buffer),
			isCompleteFile: true
		};
	} finally {
		await handle.close();
...
function formatResultProse(value) {
	...
	if (totalPages !== void 0) pagesParts.push(`${value.summary?.page_count_source === "content-list-lower-bound" ? "Parsed page lower bound" : "Total Pages"}: ${String(totalPages)}`);
	...
	if (status === "complete") footer = "\n---\n[Status: Content complete. " + pagesInfo + "Selected parsed text complete across this and preceding chunks; not a guarantee of OCR fidelity or visual coverage.]";
		...
		const mdGuidance = typeof value.cursor === "string" && value.cursor.length > 0 ? `Continue with read_pdf({ file_path: "<same file_path>", cursor: "${value.cursor}" }); omit pages/focus.` : "Full markdown artifact available in local result storage.";
	...
	if (value.diagnostics?.length) lines.push("", "Diagnostics:", ...value.diagnostics.map((d) => `- [${d.code}] (${d.scope})${d.block_id ? " [" + d.block_id + "]" : ""}${d.page === void 0 ? "" : " Page " + d.page}: ${d.message}`));
	if (value.verification_hints?.length) lines.push("", "Formula verification suggested (advisory, not a detected error):", ...value.verification_hints.map((hint) => `- [${hint.block_id}]${hint.page === void 0 ? "" : " Page " + hint.page}: inspect the original page before relying on formula symbols.`));
...
*/
function formatPageOutOfRangeMessage(totalPages) {
	const prefix = "[PAGE_OUT_OF_RANGE] Requested pages are outside the document page range";
	if (typeof totalPages === "number" && Number.isSafeInteger(totalPages) && totalPages >= 1) return `${prefix} (valid: ${totalPages === 1 ? "1" : `1-${totalPages}`}, total pages: ${totalPages})`;
	return prefix;
}
...
function encodeReadCursor(payload) {
	...
	if (payload.inline_images !== void 0 && typeof payload.inline_images !== "boolean") throw new TypeError("cursor inline_images must be a boolean");
	const canonical = {
		v: 3,
		rid: payload.rid,
		pages: payload.pages,
		...
		...payload.block === void 0 ? {} : { block: payload.block },
		...payload.query === void 0 ? {} : { query: payload.query },
		...payload.projection === void 0 ? {} : { projection: payload.projection }
	};
...
function decodeReadCursor(token) {
	...
	const obj = raw;
	if (Object.keys(obj).sort().some((key) => ![
		"focus",
		"off",
		"pages",
		"rid",
		...
		"query",
		"projection"
	].includes(key))) throw new TypeError("cursor is malformed or expired; re-read without a cursor");
	if (obj.block !== void 0 && (typeof obj.block !== "string" || !/^mr_[a-zA-Z0-9_-]+:b[1-9][0-9]*$/.test(obj.block) || obj.block.length > 160)) throw new TypeError("cursor block is malformed");
	if (obj.query !== void 0 && (typeof obj.query !== "string" || obj.query.trim() === "" || obj.query.length > 256)) throw new TypeError("cursor query is malformed");
	if (obj.block !== void 0 && obj.query !== void 0) throw new TypeError("cursor selection is malformed");
	if (obj.v !== 3) throw new TypeError("cursor is expired; re-read without a cursor");
	...
	if (obj.projection !== void 0 && (typeof obj.projection !== "string" || !/^[a-f0-9]{64}$/.test(obj.projection))) throw new TypeError("cursor projection is malformed");
	if (typeof obj.block === "string" && !obj.block.startsWith(obj.rid + ":b")) throw new TypeError("cursor block belongs to another result");
	if ((obj.block !== void 0 || obj.query !== void 0) && (focus.includes("toc") || focus.includes("artifacts"))) throw new TypeError("cursor selection cannot use toc or artifacts");
...
function cursorForRemainder(resultId, pagesLabel, focus, offset, selection = {}) {
...
//#endregion
//#region src/service/document-index.ts
const MAX_DETAIL_WARNINGS = 19;
function diagnosticId(resultId, detail) {
	return `document-index:${createHash("sha256").update([
		resultId,
		String(detail.documentOrder),
		detail.code,
		detail.location,
		detail.message
...
var WarningCollector = class {
	...
	#omitted = 0;
	#omissionScope;
	constructor(resultId, orders) {
		this.resultId = resultId;
		this.orders = orders;
	}
	forBlock(documentOrder, page) {
		const blockId = `${this.resultId}:b${String(documentOrder)}`;
		return { add: (code, location, message) => {
			if (this.orders !== void 0 && !this.orders.has(documentOrder)) return;
			...
			this.#omissionScope ??= {
				...
				...omittedDetail.page === void 0 ? {} : { page: omittedDetail.page }
			};
		} };
	}
	#sortedDetails() {
	...
	finish() {
		...
		return [...details, `[DOCUMENT_INDEX_WARNINGS_OMITTED] ${String(this.#omitted)} additional normalization warning(s) omitted.`];
	}
	finishDiagnostics() {
		const diagnostics = this.#sortedDetails().map((detail) => ({
			id: diagnosticId(this.resultId, detail),
			code: detail.code,
			scope: "chunk",
			message: detail.message,
			block_id: detail.blockId,
			...detail.page === void 0 ? {} : { page: detail.page }
		}));
		if (this.#omitted === 0 || this.#omissionScope === void 0) return diagnostics;
		...
		const detail = {
			...
			...this.#omissionScope.page === void 0 ? {} : { page: this.#omissionScope.page }
		};
		return [...diagnostics, {
			id: diagnosticId(this.resultId, detail),
			code: detail.code,
...
const KNOWN_BLOCK_TYPES = /* @__PURE__ */ new Set([
	"text",
	"title",
	"list",
	"header",
...
*/
function extractDocumentLabel(block) {
	...
	] : kind === "table" ? [...captionStrings(block.table_caption), ...captionStrings(block.caption)] : kind === "equation" ? [...captionStrings(block.equation_caption), ...captionStrings(block.caption)] : [];
	for (const caption of captions) {
		...
		if (kind === "equation") {
			...
			if (prefixed !== void 0) return prefixed.trim();
			const parenthesized = caption.match(/^\s*([（(]\s*[A-Za-z]?\d+(?:[.\-]\d+)*\s*[）)])(?=\s|[:：.、]|$)/u)?.[1]?.trim();
			if (parenthesized !== void 0) return parenthesized;
		}
	}
}
function safeStringField(source, target, field, blockLocation, warnings) {
...
function safeCaptionField(source, target, field, blockLocation, warnings) {
	const value = source[field];
	if (value === void 0) return void 0;
	if (typeof value === "string") return value;
...
function contentFromListItems(value, target, blockLocation, warnings) {
	if (!Array.isArray(value)) {
	...
	if (strings.length === 0) return void 0;
	return strings.join("\n");
}
function inlineEquation(content) {
	if (/^\$(?!\$)[\s\S]*\$$/u.test(content)) return content;
	if (/^\$\$[\s\S]*\$\$$/u.test(content)) return `$${content.slice(2, -2)}$`;
	return `$${content.match(/^\\\(([\s\S]*)\\\)$/u)?.[1] ?? content}$`;
}
...
function preferNonBlank(...values) {
	return values.find(nonBlank) ?? values.find((value) => value !== void 0);
}
function comparableText(value) {
	return value.replace(/(?:\$\$|\$|\\\(|\\\)|\\\[|\\\])/gu, "").replace(/\s+/gu, "");
}
function contentFromNestedLines(lines, blockLocation, warnings) {
...
*/
function normalizeDocumentBlockSubset(contentList, resultId, warningCollector, documentOrders) {
	return contentList.map((source, index) => {
		const documentOrder = documentOrders?.[index] ?? index + 1;
		const blockLocation = `block ${String(documentOrder)}`;
		const page = typeof source.page_idx === "number" && Number.isSafeInteger(source.page_idx) && source.page_idx >= 0 ? source.page_idx + 1 : void 0;
		const warnings = warningCollector.forBlock(documentOrder, page);
		const target = { ...source };
		delete target.document_label;
		if (source.type !== void 0 && typeof source.type !== "string") {
			delete target.type;
			warnings.add("DOCUMENT_INDEX_UNSUPPORTED_CONTENT", blockLocation, "non-string block type was omitted");
		} else if (typeof source.type === "string" && !KNOWN_BLOCK_TYPES.has(source.type.toLowerCase())) warnings.add("DOCUMENT_INDEX_UNKNOWN_BLOCK_TYPE", blockLocation, "unknown block type; only supported fields were projected, so textual coverage is not guaranteed");
		...
		for (const field of CAPTION_FIELDS) {
			...
			if (value !== void 0) safeCaptions[field] = value;
		}
		if (safeCaptions.image_caption === void 0) {
			const imageCaption = safeCaptions.img_caption ?? safeCaptions.chart_caption;
			if (imageCaption !== void 0) target.image_caption = imageCaption;
		}
		if (safeCaptions.image_footnote === void 0) {
			const imageFootnote = safeCaptions.img_footnote ?? safeCaptions.chart_footnote;
			if (imageFootnote !== void 0) target.image_footnote = imageFootnote;
		}
		const sourceType = typeof source.type === "string" ? source.type.toLowerCase() : void 0;
		const kind = blockKind(source.type);
		const listText = sourceType === "list" && source.list_items !== void 0 ? contentFromListItems(source.list_items, target, blockLocation, warnings) : void 0;
		const flatText = preferNonBlank(text, content);
		const selectedFlat = kind === "code" ? preferNonBlank(code, text, content) : kind === "table" ? preferNonBlank(tableBody, text, content) : flatText;
		let nestedText;
		if (source.lines !== void 0) {
			if (kind === "table") warnings.add("DOCUMENT_INDEX_UNSUPPORTED_CONTENT", blockLocation, "nested table lines cannot be losslessly normalized; raw fields were preserved");
			else {
				nestedText = contentFromNestedLines(source.lines, blockLocation, warnings);
				if (kind === "image" && nonBlank(nestedText)) warnings.add("DOCUMENT_INDEX_UNSUPPORTED_CONTENT", blockLocation, "nonempty nested image text was not merged; verify the original page");
			}
		}
		if (kind !== "table" && kind !== "image") {
			if (nonBlank(selectedFlat)) {
				...
				if (nonBlank(listText) && comparableText(selectedFlat) !== comparableText(listText)) warnings.add("DOCUMENT_INDEX_CONFLICTING_CONTENT", blockLocation, "nonempty flat and list text differ; flat content was kept");
				if (nonBlank(nestedText) && comparableText(selectedFlat) !== comparableText(nestedText)) warnings.add("DOCUMENT_INDEX_CONFLICTING_CONTENT", blockLocation, "nonempty flat and nested text differ; flat content was kept");
			} else if (nonBlank(listText)) {
				target.text = listText;
				if (nonBlank(nestedText) && comparableText(listText) !== comparableText(nestedText)) warnings.add("DOCUMENT_INDEX_CONFLICTING_CONTENT", blockLocation, "nonempty list and nested text differ; list content was kept");
			} else if (nonBlank(nestedText)) {
				if (kind === "code") target.code = nestedText;
				else target.text = nestedText;
			} else if (selectedFlat !== void 0) warnings.add("DOCUMENT_INDEX_EMPTY_CONTENT", blockLocation, "flat textual content is empty");
			else if (source.lines === void 0) warnings.add("DOCUMENT_INDEX_EMPTY_CONTENT", blockLocation, "no supported textual content was present");
			else warnings.add("DOCUMENT_INDEX_EMPTY_CONTENT", blockLocation, "no supported textual content was recovered");
		} else if (kind === "table" && !nonBlank(selectedFlat)) {
			if (selectedFlat !== void 0) warnings.add("DOCUMENT_INDEX_EMPTY_CONTENT", blockLocation, "flat textual content is empty");
			else if (source.lines === void 0) warnings.add("DOCUMENT_INDEX_EMPTY_CONTENT", blockLocation, "no supported textual content was present");
		}
		const normalizedProse = typeof target.text === "string" ? target.text : typeof target.content === "string" ? target.content : void 0;
		if ((sourceType === void 0 || sourceType === "text" || sourceType === "title") && normalizedProse !== void 0 && POSSIBLE_TEXT_GAP_PATTERNS.some((pattern) => pattern.test(normalizedProse))) warnings.add("DOCUMENT_INDEX_POSSIBLE_TEXT_GAP", blockLocation, "possible missing value detected; verify the original page");
		const documentLabel = extractDocumentLabel(source);
		return {
			...target,
			block_id: `${resultId}:b${String(documentOrder)}`,
			...documentLabel === void 0 ? {} : { document_label: documentLabel },
...
* The input array and its blocks are never mutated.
*/
function normalizeDocumentBlocks(contentList, resultId, warningBlockOrders) {
...
function collectBlockDiagnostics(contentList, resultId, orders) {
...
//#endregion
//#region src/service/read-delivery.ts
...
const MAX_MODEL_MARKDOWN_CHARS = 8e3;
...
function fitsReadBudget(view) {
	const json = JSON.stringify(view);
	const prose = formatResultProse(view);
	return json.length <= view.output_limit_chars && prose.length <= view.output_limit_chars && Buffer.byteLength(json, "utf8") <= 48e3 && Buffer.byteLength(prose, "utf8") <= 48e3;
}
/** Last-resort metadata degradation never changes text, status, cursor, or image records. */
function compactReadMetadata(value) {
	let view = value;
	for (const key of [
		"summary",
		"provenance",
		...
		if (view[key] === void 0) continue;
		const { [key]: _removed, ...rest } = view;
		view = {
			...rest,
			metadata_shortened: [.../* @__PURE__ */ new Set([...view.metadata_shortened ?? [], key])]
		};
	}
	return view;
}
function deliverReadChunk(options) {
	...
	if (cursor !== void 0 && cursor.projection !== projection) throw new MinerUError(failure("INVALID_REQUEST", "Cursor projection changed or expired; restart without a cursor"));
	const offset = cursor?.off ?? 0;
	if (!Number.isSafeInteger(offset) || offset < 0 || offset > text.length || offset > 0 && offset < text.length && /[\uD800-\uDBFF]/.test(text[offset - 1]) && /[\uDC00-\uDFFF]/.test(text[offset])) throw new MinerUError(failure("INVALID_REQUEST", "Cursor is outside the projection or splits a Unicode character; start over without a cursor"));
	...
	for (;;) {
		...
		const shortened = [...diagnostics.length > 6 ? ["diagnostics"] : [], ...hints.length > 4 ? ["verification_hints"] : []];
		const warnings = boundedWarnings([...offset === 0 ? base.warnings ?? [] : [], ...chunkImages.length > 20 ? ["[VISUAL_METADATA_SHORTENED] Use block_id to inspect images not listed in this chunk."] : []]);
		const { warnings: _scopeWarnings, provenance: _provenance, ...chunkBase } = base;
		const view = {
			...chunkBase,
			...offset === 0 && base.provenance ? { provenance: base.provenance } : {},
			markdown_content: content,
			content_status: partial ? "partial" : "complete",
			...partial && base.files.some((file) => file.artifacts.length > 0) ? { files: base.files.map((file) => ({
				...file,
				artifacts: [],
				artifacts_truncated: true
			})) } : {},
			cursor: partial ? cursorForRemainder(base.result_id, base.pages, focus, offset + content.length, {
				...selection,
				projection,
				inline_images: options.inlineImages ?? cursor?.inline_images ?? true
			}) : null,
			...chunkImages.length ? { ordered_images: chunkImages.slice(0, 20).map((image) => ({
				...image,
				...image.caption ? { caption: safeStringSlice(image.caption, 256) } : {}
			})) } : {},
			...
			...activeBlock ? { continuation_block: {
				block_id: activeBlock.block_id,
				...activeBlock.page === void 0 ? {} : { page: activeBlock.page },
				...activeBlock.document_label ? { document_label: activeBlock.document_label } : {}
			} } : {}
		};
		...
		target = Math.max(0, Math.floor(Math.min(target, content.length) * .75));
	}
}
//#endregion
//#region src/service/pdfjs-backend.ts
...
function positiveInteger(value, name, maximum = Number.MAX_SAFE_INTEGER) {
	if (!Number.isSafeInteger(value) || value < 1 || value > maximum) throw new TypeError(name + " must be a positive safe integer");
	return String(value);
}
function pathArgument(value, name) {
	if (typeof value !== "string" || value.length === 0 || value.includes("\0")) throw new TypeError(name + " must be a non-empty path");
	return value;
}
/**
...
*/
function buildPdfjsArgs(snapshotPath, outputPath, page, maxFileBytes) {
	return [
...
//#endregion
//#region src/service/page-renderer.ts
/** Aspect-preserving Poppler target: the longer page edge is capped at 1600 pixels. */
const PDF_PAGE_MAX_LONG_EDGE = 1600;
const MAX_RENDERED_PNG_BYTES = 8388608;
const MAX_PDFINFO_STDOUT_BYTES = 65536;
const MAX_PROCESS_STDERR_BYTES = 16384;
const MAX_RENDER_STDOUT_BYTES = 4096;
const MAX_PAGE_COUNT = 1e6;
...
const runPdfPageProcess = (request) => new Promise((resolvePromise, rejectPromise) => {
	...
	const capture = (target, kind, raw) => {
		if (terminalError !== void 0) return;
		const chunk = Buffer.isBuffer(raw) ? raw : Buffer.from(raw);
		if (kind === "stdout") stdoutBytes += chunk.byteLength;
		else stderrBytes += chunk.byteLength;
		if ((kind === "stdout" ? stdoutBytes : stderrBytes) > (kind === "stdout" ? request.maxStdoutBytes : request.maxStderrBytes)) {
	...
	const onAbort = () => failAndKill(request.signal.reason);
	...
	timeout.unref();
	const outputWatch = request.watchedOutput === void 0 ? void 0 : setInterval(() => {
		if (watching || terminalError !== void 0) return;
		watching = true;
		stat(request.watchedOutput.path).then((info) => {
			if (info.size > request.watchedOutput.maxBytes) failAndKill(new ProcessFailure("output-limit", "Rendered PNG exceeded its byte limit"));
		}, (error) => {
	...
	child.once("close", (exitCode, signal) => {
		clearTimeout(timeout);
		if (outputWatch !== void 0) clearInterval(outputWatch);
...
function invalid(message, cause) {
	return new MinerUError(failure("INVALID_REQUEST", message), cause === void 0 ? void 0 : { cause });
}
function assertInput(input) {
	if (typeof input.file_path !== "string" || input.file_path.trim() === "") throw invalid("A local PDF path is required for page rendering");
	if (!Number.isSafeInteger(input.page) || input.page < 1 || input.page > MAX_PAGE_COUNT) throw invalid("PDF page must be a positive safe integer");
	if (!Number.isSafeInteger(input.maxFileBytes) || input.maxFileBytes < 1) throw invalid("maxFileBytes must be a positive safe integer");
	if (input.expectedSha256 !== void 0 && !/^[a-fA-F0-9]{64}$/.test(input.expectedSha256)) throw invalid("expectedSha256 must be a 64-character hexadecimal SHA-256 digest");
}
...
async function checkedPathStat(path, name) {
	let info;
	try {
		info = await lstat(path, { bigint: true });
	} catch (error) {
		if (errorCode(error) === "ENOENT") throw new MinerUError(failure("FILE_NOT_FOUND", "PDF does not exist: " + name), { cause: error });
		throw error;
	}
...
async function openCheckedSource(path, name, maxFileBytes) {
	...
	try {
		...
		if (!opened.isFile() || !sameIdentity(before, opened) || !sameIdentity(opened, afterOpen)) throw invalid("PDF source changed while it was being opened: " + name);
		if (opened.size > BigInt(maxFileBytes)) throw new MinerUError(failure("FILE_TOO_LARGE", name + " exceeds the configured file-size limit"));
		return {
			handle,
			initial: opened
		};
...
async function snapshotSource(sourcePath, snapshotPath, sourceName, maxFileBytes, signal) {
	...
	try {
		await pipeline(handle.createReadStream({ autoClose: false }), limiter, createWriteStream(snapshotPath, {
			flags: "wx",
			mode: 384
		}), { signal });
		signal.throwIfAborted();
		...
		if (!sameVersion(initial, finalHandle) || !sameVersion(finalHandle, finalPath) || BigInt(bytes) !== initial.size) throw invalid("PDF source changed while it was being copied: " + sourceName);
		return hash.digest("hex");
	} finally {
		await handle.close();
	}
}
function boundedProcessResult(result, request) {
	if (result.stdout.byteLength > request.maxStdoutBytes || result.stderr.byteLength > request.maxStderrBytes) throw new MinerUError(failure("RESULT_TOO_LARGE", request.command + " output exceeded its byte limit"));
	return result;
}
async function invokeProcess(runProcess, request) {
	try {
		...
		if ([
			...
			"ENOEXEC",
			"ENOSYS"
		].includes(errorCode(error) ?? "")) throw new RendererUnavailable();
		if (error instanceof ProcessFailure && error.kind === "output-limit") throw new MinerUError(failure("RESULT_TOO_LARGE", request.command + " output exceeded its byte limit"), { cause: error });
		if (error instanceof ProcessFailure && error.kind === "timeout") throw new MinerUError(failure("PROVIDER_UNAVAILABLE", request.command + " exceeded its runtime limit", true), { cause: error });
...
var RendererUnavailable = class extends Error {};
function unavailableRenderers() {
	return new MinerUError(failure("UNSUPPORTED_OPTION", "Local PDF page rendering is unavailable: install Poppler (pdfinfo and pdftoppm) or reinstall the plugin with PDF.js and @napi-rs/canvas platform optional dependencies enabled on a supported Node/platform combination."));
}
function parsePdfjsPageCount(result) {
	...
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed) || Object.keys(parsed).length !== 1 || !("page_count" in parsed) || !Number.isSafeInteger(parsed.page_count) || typeof parsed.page_count !== "number" || parsed.page_count < 1 || parsed.page_count > MAX_PAGE_COUNT) throw invalid("PDF.js returned invalid page metadata");
...
function parsePageCount(stdout) {
	...
	if (!Number.isSafeInteger(pages) || pages < 1 || pages > MAX_PAGE_COUNT) throw invalid("PDF physical page count is outside the supported range");
	return pages;
}
async function readBoundedFile(path, maxBytes) {
	...
	try {
		...
		if (before.size > BigInt(maxBytes)) throw new MinerUError(failure("RESULT_TOO_LARGE", "Rendered PNG exceeds its byte limit"));
		const chunks = [];
		let total = 0;
		let position = 0;
		for (;;) {
			...
			if (total > maxBytes) throw new MinerUError(failure("RESULT_TOO_LARGE", "Rendered PNG exceeds its byte limit"));
			chunks.push(chunk.subarray(0, read.bytesRead));
		}
		if (!sameVersion(before, await handle.stat({ bigint: true })) || BigInt(total) !== before.size) throw invalid("Rendered PNG changed while it was being read");
...
function pngDimensions(data) {
	...
	if (bytes.byteLength < 24 || !bytes.subarray(0, 8).equals(PNG_SIGNATURE) || bytes.readUInt32BE(8) !== 13 || bytes.toString("ascii", 12, 16) !== "IHDR") throw invalid("Page renderer returned an invalid PNG");
	const width = bytes.readUInt32BE(16);
	const height = bytes.readUInt32BE(20);
	if (width < 1 || height < 1 || width > 1600 || height > 2e3 || Math.max(width, height) > 1600 || width * height > 32e5) throw new MinerUError(failure("RESULT_TOO_LARGE", "Rendered PNG dimensions exceed the page-rendering limit"));
	let offset = 8;
	let imageData = false;
	let ended = false;
	let chunks = 0;
	while (offset < bytes.length) {
		if (++chunks > 65536 || bytes.length - offset < 12) throw invalid("Page renderer returned a malformed PNG");
		...
		if (crc32(bytes.subarray(offset + 4, offset + 8 + length)) !== bytes.readUInt32BE(offset + 8 + length)) throw invalid("Page renderer returned a PNG with an invalid checksum");
...
function outputName(sourceName, page) {
	const extension = extname(sourceName);
	return (basename(sourceName, extension).replace(/[^A-Za-z0-9_.-]/g, "_").slice(0, 160) || "document") + "-page-" + String(page) + ".png";
}
function normalizeFailure(error, timedOut) {
	if (error instanceof MinerUError) return error;
	if (timedOut || error instanceof DOMException && error.name === "TimeoutError") return new MinerUError(failure("PROVIDER_UNAVAILABLE", "PDF page rendering exceeded its runtime limit", true), { cause: error });
	if (error instanceof DOMException && error.name === "AbortError") return new MinerUError(failure("CANCELLED", "PDF page rendering was cancelled", true), { cause: error });
	return new MinerUError(failure("PROVIDER_UNAVAILABLE", "Local PDF page rendering failed", true), { cause: error });
}
function createPdfPageRenderer(dependencies = {}) {
	...
	const runtimeMs = dependencies.runtimeMs ?? 45e3;
	const removeTemporaryDirectory = dependencies.removeTemporaryDirectory ?? ((path) => rm(path, {
		recursive: true,
		force: true,
		maxRetries: 2
	}));
	if (!Number.isSafeInteger(maxConcurrency) || maxConcurrency < 1 || maxConcurrency > 32) throw new TypeError("maxConcurrency must be an integer between 1 and 32");
	...
	return async (input) => {
		...
		if (extname(sourceName).toLowerCase() !== ".pdf") throw new MinerUError(failure("UNSUPPORTED_OPTION", "Original-page rendering supports PDF files only"));
		const expiresAt = performance.now() + runtimeMs;
		let timedOut = false;
		const operation = new AbortController();
		const onAbort = () => operation.abort(new DOMException("PDF page rendering was cancelled", "AbortError"));
		...
		deadline.unref();
		const remainingMs = (cap = runtimeMs) => {
		...
		try {
			...
			const sha256 = await snapshotSource(sourcePath, snapshotPath, sourceName, input.maxFileBytes, operation.signal);
			if (input.expectedSha256 !== void 0 && sha256 !== input.expectedSha256.toLowerCase()) throw invalid("PDF source hash does not match expectedSha256: " + sourceName);
			let renderer = "poppler";
			let pageCount;
			try {
				...
				if (input.page > pageCount) throw invalid("PDF page " + String(input.page) + " is outside the physical page range 1-" + String(pageCount));
				const rendered = await invokeProcess(runProcess, {
					...
					args: [
						...
						"-png",
						"-scale-to",
						String(PDF_PAGE_MAX_LONG_EDGE),
						snapshotPath,
						outputPrefix
					],
					cwd: temporaryDirectory,
					signal: operation.signal,
					timeoutMs: remainingMs(3e4),
					maxStdoutBytes: MAX_RENDER_STDOUT_BYTES,
					maxStderrBytes: MAX_PROCESS_STDERR_BYTES,
					watchedOutput: {
						path: outputPath,
						maxBytes: MAX_RENDERED_PNG_BYTES
					}
				});
				remainingMs();
				...
				try {
					rendered = await invokeProcess(runProcess, {
						command: "pdfjs",
						args: buildPdfjsArgs(snapshotPath, outputPath, input.page, input.maxFileBytes),
						cwd: temporaryDirectory,
						signal: operation.signal,
			...
			try {
				if (temporaryDirectory !== void 0) await removeTemporaryDirectory(temporaryDirectory);
			} catch {
				cleanupFailed = true;
		...
		try {
			...
			throw normalizeFailure(error, timedOut);
		}
		if (cleanupFailed) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", "Failed to remove temporary PDF page-rendering data", true));
		if (outcome === void 0) throw new MinerUError(failure("PROVIDER_UNAVAILABLE", "Local PDF page rendering produced no result", true));
		return outcome;
	};
}
const renderPdfPage = createPdfPageRenderer();
//#endregion
//#region src/service/mineru-service.ts
function reportProgress(listener, phase) {
	try {
		listener?.(phase);
	} catch {}
}
...
const MAX_SYNOPSIS_CONTENT_LIST_BYTES = 2097152;
...
async function readJsonArtifact(artifact, maxBytes, signal) {
	signal?.throwIfAborted();
...
async function readContentList(artifact, maxBytes, signal) {
	const parsed = await readJsonArtifact(artifact, maxBytes, signal);
	const container = typeof parsed === "object" && parsed !== null ? parsed : void 0;
	...
	for (const block of candidate) {
		...
		if (block.page_idx !== void 0 && (!Number.isSafeInteger(block.page_idx) || block.page_idx < 0)) throw new TypeError("content-list contains an invalid page_idx");
		if (block.type !== void 0 && typeof block.type !== "string") throw new TypeError("content-list contains an invalid type");
	}
	return candidate;
}
/** A complete contiguous MinerU pdf_info page list is stronger than the last text block's index. */
async function readLayoutPageCount(artifact, signal) {
	if (!artifact || artifact.bytes > MAX_CONTENT_LIST_BYTES) return void 0;
	const parsed = await readJsonArtifact(artifact, MAX_CONTENT_LIST_BYTES, signal);
	const pages = typeof parsed === "object" && parsed !== null ? parsed.pdf_info : void 0;
	if (!Array.isArray(pages) || pages.length === 0 || pages.length > 99999) return void 0;
	if (!pages.every((page, index) => typeof page === "object" && page !== null && !Array.isArray(page) && page.page_idx === index)) return void 0;
	return pages.length;
}
var MinerUService = class {
	...
	async prepare(session, input, signal, cacheOnly = false) {
		...
		const hit = current.storage.cacheEnabled || cacheOnly ? await this.options.results.get(cacheKey, prepared.request.requiredArtifacts, signal) : void 0;
		if (hit !== void 0) {
			const pending = {
				prepared,
				cacheKey,
	...
	fitSingleCandidate(candidate) {
		...
		view = compactReadMetadata({
			...view,
			files: strippedFiles
		});
		if (!fitsReadBudget(view)) throw new MinerUError(failure("RESULT_TOO_LARGE", "Result metadata exceeds configured model output limit"));
		return view;
	}
	async projectSingle(data, limit, cursorPayload, input = {}, signal) {
		signal?.throwIfAborted();
		...
		const markdownArtifact = data.markdownPath === void 0 ? void 0 : {
			kind: "markdown",
		...
		let rawBlocks = [];
		if (artifact) try {
			rawBlocks = await readContentList(artifact, MAX_CONTENT_LIST_BYTES, signal);
		} catch (error) {
			signal?.throwIfAborted();
			if (error instanceof MinerUError) throw error;
		...
		const blocks = normalizeDocumentBlocks(rawBlocks, data.manifest.id, /* @__PURE__ */ new Set()).blocks;
		const summary = blocks.length ? computeDocumentSummary(blocks) : void 0;
		const { toc: outline, ...counts } = summary ?? {};
		const warnings = [];
		let physicalPageCount;
		try {
			physicalPageCount = await readLayoutPageCount(data.secondaryArtifacts.find((artifact) => artifact.kind === "layout"), signal);
		} catch {
			signal?.throwIfAborted();
			warnings.push("[PAGE_COUNT_UNAVAILABLE] Layout page metadata could not be read reliably; use original page view to verify physical bounds.");
		}
		if (physicalPageCount !== void 0 && (summary?.page_count ?? 0) > physicalPageCount) {
			warnings.push("[PAGE_MAPPING_CONFLICT] Content blocks exceed layout page metadata; physical bounds are not reliable.");
			physicalPageCount = void 0;
		}
		if (physicalPageCount !== void 0) counts.page_count = physicalPageCount;
		if (counts.page_count !== void 0) counts.page_count_source = physicalPageCount === void 0 ? "content-list-lower-bound" : "layout";
		if (rawPagesSet !== void 0 && blocks.some((block) => block.page_idx === void 0 && (focusSet.has("all") || focusSet.has(getBlockCategory(block.type)) || focusSet.has("toc") && block.text_level !== void 0))) throw new MinerUError(failure("INVALID_REQUEST", "[SELECTION_UNAVAILABLE] Some selected content has no page coordinates; read without pages or use block_id/original page view"));
		if (rawPagesSet !== void 0 && summary?.page_count === void 0) throw new MinerUError(failure("INVALID_REQUEST", "[SELECTION_UNAVAILABLE] No reliable physical page coordinates are available"));
		...
		const pagesLabel = rawPagesSet === void 0 ? void 0 : narrowed.pagesLabel;
		const pagesSet = narrowed.pagesSet;
		if (rawPagesSet !== void 0 && physicalPageCount === void 0) warnings.push("[PAGE_COUNT_LOWER_BOUND] Physical page count is unknown; parsed page_count is a lower bound. An empty selection is not proof that the requested page is blank or absent.");
		...
		const base = {
			...
			files: [{
				file_id: data.fileId,
				name: data.fileName,
				artifacts: artifactList.slice(0, 20),
				...artifactList.length > 20 ? { artifacts_truncated: true } : {}
			}],
			...
			provenance: {
				provider: data.manifest.producer.providerId,
				model: data.manifest.request.semantics.model,
				parse_method: data.manifest.request.semantics.parseMethod,
			...
			...artifactsRequested ? {
				...
				...data.markdownPath ? { markdown_path: data.markdownPath } : {}
			} : {},
		...
		let images = [];
		let ranges = [];
		let verificationHints = [];
		if (blocks.length) {
			const selected = blocks.filter((block) => {
				const page = typeof block.page_idx === "number" ? block.page_idx + 1 : void 0;
				return (pagesSet === void 0 || page !== void 0 && pagesSet.has(page)) && (focusSet.has("all") || focusSet.has(getBlockCategory(block.type)));
			});
			if (blockId !== void 0 && !blocks.some((block) => block.block_id === blockId)) throw new MinerUError(failure("INVALID_REQUEST", "[BLOCK_NOT_FOUND] This block does not belong to the current parsed result; search again"));
			const matching = blockId === void 0 ? selected : selected.filter((block) => block.block_id === blockId);
			if (blockId !== void 0 && matching.length === 0) throw new MinerUError(failure("INVALID_REQUEST", "[BLOCK_NOT_FOUND] Block is outside the requested pages/focus"));
			verificationHints = matching.filter((block) => [
				"equation",
				"interline_equation",
				"inline_equation"
			].includes((block.type ?? "").toLowerCase())).map((block) => ({
				reason: "formula",
				block_id: block.block_id,
				view: "page",
				...typeof block.page_idx === "number" ? { page: block.page_idx + 1 } : {}
			}));
			...
			if (query !== void 0) {
				...
				for (const block of matching) {
					signal?.throwIfAborted();
					const projected = extractBlocksMarkdown([block], void 0, /* @__PURE__ */ new Set(["all"]), imageArtifacts).text;
				...
				const projected = extractBlocksMarkdown(matching, void 0, /* @__PURE__ */ new Set(["all"]), imageArtifacts);
				fullText = projected.text;
				images = projected.orderedImages;
				ranges = projected.ranges;
			}
			if (focusSet.has("toc")) {
				const headings = (outline ?? []).filter((heading) => pagesSet === void 0 || heading.page !== void 0 && pagesSet.has(heading.page));
				const tocRanges = [];
				const tocText = formatTocMarkdown(headings, { pageRange: pagesLabel }, tocRanges);
				if (fullText) ranges = ranges.map((range) => ({
					...range,
					start: range.start + tocText.length + 2,
					locator_end: range.locator_end + tocText.length + 2,
					end: range.end + tocText.length + 2
				}));
				ranges = [...tocRanges, ...ranges];
				fullText = fullText ? tocText + "\n\n" + fullText : tocText;
			}
		} else {
			if (blockId !== void 0 || query !== void 0 || rawPagesSet !== void 0 || !focusSet.has("all") && !focusSet.has("toc")) throw new MinerUError(failure("INVALID_REQUEST", "[SELECTION_UNAVAILABLE] No content-list mapping; use unfiltered reading or original page view"));
		...
		const scopeDiagnostics = warnings.map((warning) => {
			const code = /^\[([^\]]+)\]/.exec(warning)?.[1] ?? "PAGE_SELECTION_ADJUSTED";
			const scope = /PAGE_COUNT_LOWER_BOUND|PAGE_SELECTION/.test(code) ? "selection" : "document";
			return {
				id: createHash("sha256").update(data.manifest.id + "|" + scope + "|" + warning).digest("hex").slice(0, 24),
				code,
				scope,
				message: warning.replace(/^\[[^\]]+\]\s*/, "")
			};
		});
		return deliverReadChunk({
			base: {
				...base,
				...warnings.length ? { warnings: boundedWarnings(warnings) } : {}
			},
			...
			inlineImages: input.inline_images ?? cursorPayload?.inline_images ?? true,
			verificationHints,
			scopeDiagnostics,
	...
	createWaitSignal(signal, pollTimeoutMs) {
		...
		if (timeout !== void 0 && (!Number.isSafeInteger(timeout) || timeout <= 0 || timeout > MAX_POLL_TIMEOUT_MS)) throw new MinerUError(failure("INVALID_REQUEST", "poll timeout is outside the supported range"));
		const controller = new AbortController();
	...
	async ensureParsed(session, input, signal, onProgress) {
		if (input.cursor !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "A parse summary cannot resume a read cursor; use read_pdf"));
	...
	async previewPage(session, input, signal) {
		...
		return {
			...page,
			result_id: asResultId("mr_page_" + page.sha256.slice(0, 32) + "_" + page.page),
			file_id: createFileId(page.sha256),
			output_limit_chars: this.config().output.maxInlineChars
		};
	}
	...
	async projectSummary(data, signal) {
		signal.throwIfAborted();
		...
		if (artifact !== void 0) try {
			...
			if (blocks.length > 0) {
				...
				const toc = headings.slice(0, MAX_SYNOPSIS_HEADINGS).map((heading) => ({
					...heading,
					title: safeStringSlice(heading.title, MAX_SYNOPSIS_TITLE_CHARS)
				}));
				summary = {
					...full,
					toc
				};
				if (headings.length > toc.length || headings.some((heading) => heading.title.length > MAX_SYNOPSIS_TITLE_CHARS)) warnings.push("Outline shortened for the parse summary; use read_pdf to inspect the complete outline.");
			}
		} catch {
			signal.throwIfAborted();
	...
	async resolveParsedResult(session, input, signal, pollTimeoutMs, onProgress) {
		if (input.query !== void 0 && (typeof input.query !== "string" || input.query.trim() === "" || input.query.length > 256)) throw new MinerUError(failure("INVALID_REQUEST", "query must contain 1–256 characters"));
		if (input.block_id !== void 0 && (typeof input.block_id !== "string" || input.block_id.length > 160 || !/^mr_[a-zA-Z0-9_-]+:b[1-9][0-9]*$/.test(input.block_id))) throw new MinerUError(failure("INVALID_REQUEST", "block_id must be an exact ID returned by read_pdf"));
		if (input.query !== void 0 && input.block_id !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "query and block_id cannot be combined"));
		if (input.query !== void 0 || input.block_id !== void 0) {
			let focus;
			try {
				focus = normalizeFocusSelection(input.focus);
			} catch (error) {
				throw new MinerUError(failure("INVALID_REQUEST", "Invalid focus"), { cause: error });
			}
		...
		if (input.cursor !== void 0) {
			...
			if (input.pages !== void 0 || input.focus !== void 0 || input.block_id !== void 0 || input.query !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "pages and focus must be omitted when cursor is provided, as must block_id and query"));
		}
		...
		const { pending } = await this.prepare(session, effectiveInput, signal, cursorPayload !== void 0);
...
async function readUtf8Bounded(path, maxBytes, label, signal) {
	const decoder = new TextDecoder$1("utf-8", { fatal: true });
...
//#endregion
//#region src/adapters/dsh-document-source.ts
function parseDocumentSource(args) {
	if (args.file_path !== void 0 === (args.attachment_id !== void 0)) throw new MinerUError(failure("INVALID_REQUEST", "Exactly one of file_path or attachment_id is required"));
	if (args.file_path !== void 0) {
		if (typeof args.file_path !== "string" || args.file_path.trim() === "") throw new MinerUError(failure("INVALID_REQUEST", "file_path must be a non-empty string"));
		return { file_path: args.file_path.trim() };
	}
	if (typeof args.attachment_id !== "string" || !/^(?:sha256:)?[a-fA-F0-9]{8,64}$/.test(args.attachment_id.trim())) throw new MinerUError(failure("INVALID_REQUEST", "attachment_id must be a SHA-256 digest or an unambiguous prefix of 8–64 hexadecimal characters, optionally prefixed with sha256:"));
	return { attachment_id: args.attachment_id.trim().toLowerCase() };
}
function* fileReferences(blocks) {
	for (const block of blocks) if (block.type === "file") yield block.attachment;
}
...
function resolveDocumentPath(source, session, attachments) {
	if (source.file_path !== void 0) return source.file_path;
	if (attachments === void 0) throw new MinerUError(failure("UNSUPPORTED_OPTION", "Attachment input requires the DSH attachment service; use file_path instead"));
	const prefix = source.attachment_id.replace(/^sha256:/, "");
	const matches = /* @__PURE__ */ new Map();
	for (const message of session.deriveMessages()) for (const ref of fileReferences(message.content)) {
		const id = String(ref.attachmentId);
		if (id.slice(7).startsWith(prefix) && !matches.has(id)) matches.set(id, ref);
	}
	if (matches.size === 0) throw new MinerUError(failure("INVALID_REQUEST", "No file attachment matches attachment_id on the current session surface"));
	if (matches.size > 1) throw new MinerUError(failure("INVALID_REQUEST", "attachment_id is ambiguous; use a longer prefix or the full SHA-256 ID"));
	const ref = matches.values().next().value;
	const path = attachments.fileHostPath(ref);
	if (path === void 0) throw new MinerUError(failure("UNSUPPORTED_OPTION", "The DSH attachment backend does not provide a host file path; use file_path instead"));
	return path;
}
//#endregion
//#region src/tools.ts
...
const inlinedImageViewSchema = {
	...
	properties: {
		...
		figure: {
			type: "integer",
			description: "Attachment selection index only; use document_label for the original paper figure number."
		}
	},
	additionalProperties: false
};
...
const documentSourceParameters = {
	file_path: {
		type: "string",
		description: "Path of the local PDF document. Provide exactly one of file_path or attachment_id."
	},
	attachment_id: {
		type: "string",
		description: "File attachment on the current session surface: full SHA-256 ID or unique 8–64 hex-character digest prefix, with optional sha256: prefix. Uses the DSH host file path; not a URL or filesystem path. Mutually exclusive with file_path."
	}
};
const asyncParseParameters = documentSourceParameters;
const readPdfParameters = {
	...documentSourceParameters,
	view: {
		type: "string",
		enum: ["content", "page"],
		description: "content (default) reads parsed blocks; page renders one original PDF page locally without invoking a Provider. Requires exactly one page and an image-capable model."
	},
	block_id: {
		type: "string",
		description: "Read one exact stable block ID from a prior result; IDs are bound to that parsed result. Cannot combine with query or cursor."
	},
	query: {
		type: "string",
		description: "Case-insensitive literal search (1–256 characters) in selected parsed blocks. Returns bounded snippets and block IDs, not the full matching blocks. Use block_id to read a hit."
	},
	expected_sha256: {
		type: "string",
		description: "Optional source_sha256 from a parsed result. In page view, reject changed sources before rendering."
	},
	pages: {
		oneOf: [
			{
...
function fitPostImageBudget(value) {
	...
	const warn = (message) => {
		fitted = {
			...fitted,
			warnings: boundedWarnings([message, ...fitted.warnings ?? []])
		};
	};
	...
	if (fitted.summary !== void 0 || fitted.toc !== void 0) {
		const { summary: _summary, toc: _toc, ...rest } = fitted;
		fitted = {
			...rest,
			metadata_shortened: [.../* @__PURE__ */ new Set([...rest.metadata_shortened ?? [], "summary"])]
		};
	...
	if (fitted.files.some((file) => file.artifacts.length > 0)) {
		fitted = {
			...
			files: fitted.files.map((file) => ({
				...file,
				artifacts: [],
				...file.artifacts.length ? { artifacts_truncated: true } : {}
			}))
		};
...
function parsePollTimeout(value) {
	if (value === void 0) return void 0;
	if (typeof value !== "number" || !Number.isSafeInteger(value) || value < 1 || value > 864e5) throw new MinerUError(failure("INVALID_REQUEST", "poll_timeout_ms must be an integer from 1 to 86400000"));
	return value;
}
function requireAgent(exec) {
	const agent = exec.agent;
	if (agent === void 0 || agent.session == null) throw new MinerUError(failure("UNAUTHENTICATED_SESSION", "MinerU operations require an authenticated agent session (UNAUTHENTICATED_SESSION)"));
	return agent;
}
...
const ASYNC_PARAMETER_FIELDS = /* @__PURE__ */ new Set(["file_path", "attachment_id"]);
function resolveToolInput(input, agent, attachments) {
	const { file_path: _path, attachment_id: _id, ...options } = input;
	return {
		...options,
		file_path: resolveDocumentPath(input, agent.session, attachments)
	};
}
...
function parseReadInput(args) {
	...
	if (obj.view !== void 0 && obj.view !== "content" && obj.view !== "page") throw new MinerUError(failure("INVALID_REQUEST", "view must be content or page"));
	if (obj.view === "page") {
		for (const key of [
			"focus",
			"cursor",
			...
			"inline_images",
			"poll_timeout_ms"
		]) if (obj[key] !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "page view accepts only one source (file_path or attachment_id), pages, and optional expected_sha256"));
		let selected;
		try {
			selected = normalizePageSelection(obj.pages);
		} catch {
			throw new MinerUError(failure("INVALID_REQUEST", "page view requires exactly one positive physical page"));
		}
		if (selected === void 0 || selected.size !== 1) throw new MinerUError(failure("INVALID_REQUEST", "page view requires exactly one physical page in pages"));
		if (obj.expected_sha256 !== void 0 && (typeof obj.expected_sha256 !== "string" || !/^[a-f0-9]{64}$/.test(obj.expected_sha256))) throw new MinerUError(failure("INVALID_REQUEST", "expected_sha256 must be a lowercase SHA-256 digest"));
		return {
			input: source,
			view: "page",
			page: [...selected][0],
			...typeof obj.expected_sha256 === "string" ? { expected_sha256: obj.expected_sha256 } : {}
		};
	}
	if (obj.expected_sha256 !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "expected_sha256 is only supported in page view"));
	if (obj.block_id !== void 0 && (typeof obj.block_id !== "string" || obj.block_id.length > 160 || !/^mr_[a-zA-Z0-9_-]+:b[1-9][0-9]*$/.test(obj.block_id))) throw new MinerUError(failure("INVALID_REQUEST", "block_id must be an exact ID returned by read_pdf"));
	if (obj.query !== void 0 && (typeof obj.query !== "string" || obj.query.trim() === "" || obj.query.length > 256)) throw new MinerUError(failure("INVALID_REQUEST", "query must contain 1–256 characters"));
	if (obj.block_id !== void 0 && obj.query !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "block_id and query cannot be combined"));
	if ((obj.query !== void 0 || obj.block_id !== void 0) && [...normalizeFocusSelection(obj.focus)].some((f) => f === "toc" || f === "artifacts")) throw new MinerUError(failure("INVALID_REQUEST", "query/block_id cannot be combined with toc or artifacts focus"));
	let inline_images;
	...
	if (obj.cursor !== void 0) {
		...
		if (obj.pages !== void 0 || obj.focus !== void 0 || obj.block_id !== void 0 || obj.query !== void 0) throw new MinerUError(failure("INVALID_REQUEST", "pages and focus must be omitted when cursor is provided, as must block_id and query"));
		try {
...
function nativeSuccessOutcome(value) {
	return {
		status: "completed",
		detail: "completed",
		result: formatSingleSummaryProse(value)
	};
}
var BoundedImageReadError = class extends Error {
	status;
	bytesRead;
	constructor(status, message, bytesRead) {
		super(message);
...
async function readImageBounded(path, remainingBytes, signal) {
	...
	try {
		...
		if (!details.isFile() || details.size > 8388608) throw new BoundedImageReadError("budget", "image exceeds inline budget", consumed);
...
async function inlineImagesForSingleResult(view, attachments, maxInlineImages, signal) {
	...
	for (let index = 0; index < declared.length; index++) {
		...
		if (index >= maxInlineImages || mediaType === void 0 || item.path === "") {
			statuses[index] = {
				...statuses[index],
				status: index >= maxInlineImages ? "omitted" : "unsupported"
			};
			continue;
		}
	...
	for (const item of candidates) {
		...
		try {
			...
			if (imageBytes.length > 8388608 || actualTotalBytes > 25165824) {
				statuses[item.index] = {
					...statuses[item.index],
					status: "omitted"
				};
			...
			inlined.push({
				...
				...statuses[item.index].page === void 0 ? {} : { page: statuses[item.index].page },
			...
			if (error instanceof BoundedImageReadError) {
				...
				statuses[item.index] = {
					...statuses[item.index],
					status: error.status === "budget" ? "omitted" : "failed"
				};
			} else statuses[item.index] = {
				...statuses[item.index],
				status: "failed"
			};
		}
...
function registerTools(ctx, getService, accessGate, getOutputConfig = () => DEFAULT_OUTPUT_CONFIG, getAttachments = () => ctx.get("attachments")) {
	const disposers = [];
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			const input = resolveToolInput(parseAsyncInput(args).input, agent, getAttachments());
			...
			return {
				job_id: jobs.start({
					...
					outputLimitBytes: 48e3,
					run: (job) => {
						const done = withStorageAccess(() => getService().ensureParsed(agent.session, input, controller.signal, (phase) => job.updateProgress(phase)), controller.signal).then((value) => nativeSuccessOutcome(value)).catch((error) => {
							...
							return {
								...
								result: "[" + normalized.code + "] " + normalized.message
							};
						});
						const invocation = {
							controller,
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		output: {
			...
			presentationMeta: (_args, value) => {
				...
				return {
					...
					...single.renderer === void 0 ? {} : { renderer: single.renderer },
					source: single.source,
					cache_hit: single.cache_hit,
					...single.manifest_path === void 0 ? {} : { manifest_path: single.manifest_path },
					...
					...single.summary !== void 0 ? { summary: {
						...
						...single.summary.page_count_source !== void 0 ? { page_count_source: single.summary.page_count_source } : {},
		...
		execute: async (args, exec) => {
			...
			if (parsed.view === "page") {
				...
				if (!supportsImage || attachments === void 0) throw new MinerUError(failure("UNSUPPORTED_OPTION", "Original page view requires an image-capable model and attachment support"));
				const page = await getService().previewPage(agent.session, {
					file_path: input.file_path,
					page: parsed.page,
					...parsed.expected_sha256 ? { expectedSha256: parsed.expected_sha256 } : {}
				}, exec.signal);
				exec.signal.throwIfAborted();
				const ref = await attachments.saveImage({
					data: page.data,
				...
				exec.signal.throwIfAborted();
				if (!Number.isSafeInteger(ref.bytes) || ref.bytes < 0 || ref.bytes > 8388608) throw new MinerUError(failure("RESULT_TOO_LARGE", "Rendered page attachment exceeds the image limit"));
				return fitPostImageBudget({
					state: "completed",
					source: "local",
					cache_hit: false,
					...
					content_status: "not_requested",
					cursor: null,
					output_limit_chars: page.output_limit_chars,
					source_sha256: page.sha256,
					pages: String(page.page),
					summary: {
						page_count: page.page_count,
						page_count_source: page.renderer === "pdfjs" ? "pdfjs" : "pdfinfo"
					},
					inlined_images: [{
						attachment_id: String(ref.attachmentId),
						name: ref.name ?? "page-" + page.page + ".png",
						media_type: ref.mediaType,
						bytes: ref.bytes,
						page: page.page,
						...ref.width === void 0 ? {} : { width: ref.width },
			...
			const shouldInline = inline_images === true && focusIncludesImages && supportsImage && attachments !== void 0;
			return await withStorageAccess(async () => {
				...
				const processed = shouldInline && attachments ? await inlineImagesForSingleResult(rawResult, attachments, maxInlineImages, exec.signal) : rawResult;
				const listed = processed.ordered_images?.length ?? 0;
				const attached = processed.inlined_images?.length ?? 0;
				return fitPostImageBudget(listed ? {
					...processed,
					visuals: {
						listed,
						attached,
						omitted: listed - attached,
						scope: "chunk"
					},
...
function isLoopbackAuthority(host) {
	...
	if (match === null || match[0] !== host || match[2] !== void 0 && Number(match[2]) > 65535) return false;
...
*/
function registerLoopbackRpc(ctx, channel, handler) {
	const webServer = ctx.webServer;
	return ctx.extend({ webServer: { register(route) {
...
function registerRpc(ctx, deps) {
	...
	return registerLoopbackRpc(ctx, RPC_CHANNEL, async (endpoint, payload, signal) => {
		try {
			switch (endpoint) {
...
const Config = z.object({
	schemaVersion: z.union([z.const(1), z.const(2)]).volatile(),
	...
	output: z.object({
		maxInlineChars: z.number(),
		maxInlineImages: z.number()
	}).volatile(),
	limits: z.object({
...
function configSnapshot(value) {
	...
	if (typeof value === "object" && value !== null) return Object.fromEntries(Object.entries(value).map(([key, child]) => [key, configSnapshot(child)]));
	return value;
}
function liveConfig(config) {
	...
	return {
		...
		storage: {
			...
			stagingTtlMs: storage.stagingTtlMs
		}
	};
}
...
async function apply(ctx, entryConfig = {}) {
	const runtimeConfig = () => parseConfigWithMigration(configSnapshot(ctx.fiber?.config ?? entryConfig)).config;
	...
	const validateParsedRuntimeConfig = (next) => {
	...
	const validateRuntimeConfig = (value) => validateParsedRuntimeConfig(parseConfig(value));
	...
	try {
		...
		const results = new ResultRepository(paths, {
			...
			maxJsonValidationBytes: Math.min(initialConfig.limits.maxZipEntryBytes, 67108864)
		}, lock);
		await results.cleanupStaging(initialConfig.storage.stagingTtlMs, operationRegistry.activeOperationIds(), startup.signal);
		startup.signal.throwIfAborted();
		...
		ctx.inject(["connection", "webServer"], (connectionCtx) => {
			return registerRpc(connectionCtx, {
				...
				setConfig: async (value) => {
					...
					if (namespace === void 0) throw new Error("MinerU configuration requires a Loader profile entry");
					await settings.replace(namespace, liveConfig(next));
					return runtimeConfig();
				},
				probe: async (provider, signal) => service.probe(signal, provider === void 0 ? void 0 : parseDraftProvider(provider, runtimeConfig())),
				maintenance
			});
		});
		let disposing;
...
//#endregion
export { ARTIFACT_KINDS, CACHE_KEY_SPEC_VERSION, CANONICAL_PARSE_REQUEST_SCHEMA_VERSION, Config, DEFAULT_OUTPUT_CONFIG, DEFAULT_PARSE_DEFAULTS, DEFAULT_POLLING_CONFIG, DEFAULT_RETRY_CONFIG, DEFAULT_RETRY_POLICY, DEFAULT_SECURITY_LIMITS, DEFAULT_STORAGE_OPTIONS, FOCUS_KINDS, MAX_INLINE_IMAGE_BUDGET, MINERU_CONFIG_SCHEMA_VERSION, MINERU_RESULT_MANIFEST_SCHEMA_VERSION, MIN_INLINE_IMAGE_BUDGET, MinerUError, MinerUService, OfficialV4Provider, ProviderHttpClient, RESULT_SCHEMA_VERSION, SelfHostedV2Provider, StorageMaintenanceService, apply, asCacheKey, asFileId, asOperationId, asProviderConfigId, asResultId, asSessionId, assertSafePathSegment, cacheClearConfirmationToken, calculateBackoffDelay, computeDocumentSummary, createFileId, createHttpStatusError, createOperationId, createStructuredDiagnosticSink, defaultMinerUConfig, defaultProviderConfig, defaultSleep, detectBloatedSettingsOps, emitDiagnostic, executeWithRetry, extractBlocksMarkdown, extractErrorMessage, extractMarkdownHeadings, failure, fallbackExtractFromMarkdown, findMarkdownArtifactPath, formatCaption, formatPageOutOfRangeMessage, formatResultProse, formatSingleSummaryProse, formatTocMarkdown, getBlockCategory, getRasterMediaType, inject, isRetryableError, isRetryableHttpStatus, mergeRetryOptions, name, narrowPageSelection, normalizeArtifactKinds, normalizeFocusSelection, normalizePageRanges, normalizePageSelection, parseConfig, parseConfigWithMigration, parseMinerUResultManifest, parseRetryAfter, providerById, pruneConfigToDiff, readBoundedResponseText, readMarkdownFile, resolveProviderUrl, resolveRetryPolicy, resultIdForCacheKey, safeStringSlice, sanitizeDiagnostic, throwMinerU, toMinerUFailure, truncateAtCleanBoundary, validateProviderCapabilities };
