		...
		const executeOnce = async () => {
			...
			try {
				...
				if (envelope.code !== 0) {
					...
					if (businessValidation === "probe" && normalizedCode === "BATCH_NOT_FOUND") return parsed;
...
function projectProviderRef(ref, fileId) {
...
function projectProviderSnapshot(snapshot, fileId) {
	const files = snapshot.files.filter((file) => file.fileId === fileId);
	const file = files[0];
	if (file === void 0) throw new TypeError("Provider batch snapshot does not contain participant file");
	return {
		state: file.state === "completed" || file.state === "failed" ? file.state : "processing",
		files,
		...snapshot.rawState === void 0 ? {} : { rawState: snapshot.rawState },
		...snapshot.queuedAhead === void 0 ? {} : { queuedAhead: snapshot.queuedAhead }
	};
}
...
var BatchCoordinator = class {
	...
	async execute() {
		...
		try {
			...
			const submission = await this.options.resolved.provider.submit(request, participants.map((participant) => participant.source), {
				...context,
			...
			await Promise.all(participants.map(async (participant) => {
				const fileId = participant.request.files[0].fileId;
				participant.operation.markSubmitted({
					state: projectProviderSnapshot(submission, fileId).state,
					ref: projectProviderRef(fullRef, fileId)
				});
			}));
			...
			while (snapshot.files.some((file) => file.state !== "completed" && file.state !== "failed")) {
				...
				await Promise.all(participants.map((participant) => participant.snapshot(projectProviderSnapshot(snapshot, participant.request.files[0].fileId))));
			}
			const collection = await this.options.resolved.provider.collect(fullRef, request, this.options.sink, await this.options.createContext(this.controller.signal));
			const byFile = new Map(collection.files.map((file) => [file.fileId, file]));
			await Promise.all(participants.map(async (participant) => {
				const fileId = participant.request.files[0].fileId;
				const file = byFile.get(fileId);
				const outcome = file === void 0 ? await participant.failed(/* @__PURE__ */ new TypeError("Provider collection omitted a batch participant")) : await participant.collected(file);
...
function fileViews(job, includeJobId = false) {
...
function aggregateState(jobs) {
	...
	if (jobs.every((job) => isTerminalJobState(job.state))) return jobs.some((job) => job.state === "completed" || job.state === "partially-completed") ? "partially-completed" : "failed";
...
var MinerUService = class {
	...
	async callContext(config, signal, operationId, onAccepted, allowMissingCredential = false) {
		signal.throwIfAborted();
	...
	async probe(signal, draft) {
		...
		const result = await resolved.provider.probe(await this.callContext(resolved.config, signal, void 0, void 0, true));
		return {
	...
	async submit(session, input, signal) {
		...
		return {
			kind: "batch",
			state: aggregateState(jobs),
			jobs: jobs.map(submitView)
		};
	}
	async submitJobs(session, input, signal) {
		...
		for (let index = 0; index < prepared.request.files.length; index++) {
			...
			const job = hit === void 0 ? this.newJob(sessionId, one, active, compatibility, cacheKey, { kind: "provider" }, void 0, "queued") : this.newJob(sessionId, one, active, compatibility, cacheKey, { kind: "cache-hit" }, hit.id, "completed");
			await this.options.jobs.create(session, job);
			this.diagnostic({
				level: "info",
				phase: hit === void 0 ? "job-created" : "cache-hit",
		...
		for (const item of misses) {
			const reserved = this.options.operations.reserve(item.cacheKey, active.config.id, current.polling.operationTimeoutMs);
			item.operation = reserved.operation;
			item.created = reserved.created;
		}
		await Promise.all(misses.map(async (item) => {
			const operation = item.operation;
			if (operation === void 0) throw new TypeError("Missed submission has no shared operation");
			if (!item.created) item.job = await this.options.jobs.update(session, item.job.id, (record) => ({
				...record,
				resolution: {
			...
			this.diagnostic({
				level: "debug",
				phase: "shared-operation",
				provider: active.provider.id,
				jobId: item.job.id,
				operationId: operation.id,
				waiterCount: operation.waiters.size
			});
		}));
		...
		if (producers.length === 1) {
			...
			this.options.operations.start(item.operation, (operation) => this.runOperation(operation, item.prepared, active, compatibility));
		} else if (producers.length > 1) this.startBatch(producers, active, compatibility, current);
		await Promise.all(misses.map(async (item) => {
			const operation = item.operation;
			if (operation === void 0) throw new TypeError("Missed submission has no shared operation");
			try {
	...
	async syncAcceptedRef(session, jobId, ref) {
		return this.options.jobs.update(session, jobId, (current) => {
			...
			return {
				...
				files: current.files.map((file) => ({
					...file,
					state: file.state === "queued" ? "uploading" : file.state
				}))
			};
		});
	}
	async syncSubmission(session, jobId, submitted) {
		return this.options.jobs.update(session, jobId, (current) => {
			if (isTerminalJobState(current.state)) return current;
			const resolution = submitted.ref === void 0 ? current.resolution : withProviderRef(current.resolution, submitted.ref);
			if (submitted.resultId !== void 0) return {
				...current,
				state: "completed",
				resolution,
	...
	async replayOperation(session, jobId, operation) {
		if (operation.acceptedRef !== void 0) await this.syncAcceptedRef(session, jobId, operation.acceptedRef);
		if (operation.submittedValue !== void 0) await this.syncSubmission(session, jobId, operation.submittedValue);
		const outcome = operation.settledValue;
		if (outcome?.resultId !== void 0) await this.syncSubmission(session, jobId, {
			state: outcome.state,
				...
				return {
					...
					accepted: async (ref) => {
						await this.updateWaiters(operation, (job) => isTerminalJobState(job.state) ? job : {
							...job,
							state: "uploading",
							resolution: withProviderRef(job.resolution, ref),
							files: job.files.map((status) => ({
								...status,
								state: "uploading"
							}))
					...
					snapshot: async (snapshot) => {
						...
						await this.updateWaiters(operation, (job) => isTerminalJobState(job.state) ? job : {
							...job,
							state: snapshot.state === "failed" ? "failed" : snapshot.state === "completed" ? "collecting" : "processing",
							files: this.snapshotFiles(job, snapshot),
							...snapshotFailure === void 0 ? {} : { failure: snapshotFailure }
						});
					},
	...
	async runOperation(operation, prepared, resolved, compatibility, recoveryRef) {
		...
		try {
			const cached = recoveryRef !== void 0 || this.config().storage.cacheEnabled ? await this.options.results.get(operation.cacheKey, prepared.request.requiredArtifacts, operation.controller.signal) : void 0;
			if (cached !== void 0) {
				await this.updateWaiters(operation, (current) => ({
					...current,
					state: "completed",
					resolution: ref === void 0 ? { kind: "cache-hit" } : withProviderRef(current.resolution, ref),
					resultId: cached.id,
					files: current.files.map((file) => ({
						...file,
						state: "completed",
			...
			if (ref === void 0) {
				...
				const submission = await resolved.provider.submit(prepared.request, prepared.sources, await this.callContext(resolved.config, operation.controller.signal, operation.id, async (accepted) => {
					ref = accepted;
			...
			} else snapshot = await resolved.provider.inspect(ref, await this.callContext(resolved.config, operation.controller.signal, operation.id));
			if (ref === void 0) throw new TypeError("Provider submission did not produce a durable reference");
			...
			if (snapshot.state === "failed") {
				this.diagnostic({
					level: submissionFailure.retryable ? "warn" : "error",
					phase: "failed",
					provider: resolved.provider.id,
					operationId: operation.id,
					durationMs: Date.now() - startedAt,
					...
					retryable: submissionFailure.retryable
				});
				return { state: "failed" };
			}
			this.diagnostic({
			...
			while (snapshot.state !== "completed" && snapshot.state !== "partially-completed") {
				...
				const pollingFailure = snapshot.files.find((file) => file.failure)?.failure ?? failure("REMOTE_PARSE_FAILED", "Remote parse failed");
				await this.updateWaiters(operation, (current) => isTerminalJobState(current.state) ? current : {
					...current,
					state: snapshot.state === "failed" ? "failed" : current.state === "collecting" ? "collecting" : "processing",
					resolution: withProviderRef(current.resolution, durableRef),
				...
				if (snapshot.state === "failed") {
					this.diagnostic({
						level: pollingFailure.retryable ? "warn" : "error",
						phase: "failed",
						provider: resolved.provider.id,
						operationId: operation.id,
						durationMs: Date.now() - startedAt,
						bytes: requestBytes,
						waiterCount: operation.waiters.size,
						errorCode: pollingFailure.code,
						retryable: pollingFailure.retryable
					});
					return { state: "failed" };
				}
			}
			...
			const collection = await resolved.provider.collect(durableRef, prepared.request, transaction, await this.callContext(resolved.config, operation.controller.signal, operation.id));
			const file = prepared.request.files[0];
			...
			if (collected === void 0 || collected.failure !== void 0) {
				...
				this.diagnostic({
					level: cause.retryable ? "warn" : "error",
					phase: "failed",
					provider: resolved.provider.id,
					operationId: operation.id,
					durationMs: Date.now() - startedAt,
	...
	async status(session, jobId, signal) {
		...
		const acquired = this.options.operations.acquire(job.cacheKey, job.providerConfigId, this.config().polling.operationTimeoutMs, (operation) => this.runOperation(operation, prepared, resolved, job.providerCompatibilityKey, ref));
		...
		this.diagnostic({
			...
			waiterCount: acquired.operation.waiters.size
		});
		try {
	...
	async batchEnvelope(jobs, signal, timedOut = false) {
		const children = await Promise.all(jobs.map(async (job) => {
			if (job.resultId !== void 0 && (job.state === "completed" || job.state === "partially-completed")) {
				const manifest = await this.options.results.get(job.cacheKey, job.request.requiredArtifacts, signal);
				if (manifest === void 0 || manifest.id !== job.resultId) throw new MinerUError(failure("CACHE_EVICTED", "Published MinerU result is missing or corrupt"));
				return this.projectResult(job, manifest);
			}
		...
		return {
			...
			state: aggregateState(jobs),
			jobs: children,
			...timedOut ? { poll_timed_out: true } : {}
		};
	}
	async parseBatchDocument(session, initial, signal, pollTimeoutMs) {
		let jobs = initial;
		if (jobs.every((job) => isTerminalJobState(job.state))) return this.batchEnvelope(jobs, signal);
		const active = (await Promise.all(jobs.filter((job) => !isTerminalJobState(job.state)).map(async (job) => {
		...
		if (!Number.isSafeInteger(timeout) || timeout <= 0 || timeout > MAX_POLL_TIMEOUT_MS$1) throw new MinerUError(failure("INVALID_REQUEST", "poll timeout is outside the supported range"));
...
var SharedOperation = class {
	...
	markAccepted(ref) {
		if (this.accepted === void 0) this.accepted = ref;
	}
	markSubmitted(value) {
		if (this.submitted) return;
		this.submitted = true;
		this.submissionValue = value;
		if (value.ref !== void 0) this.markAccepted(value.ref);
	...
	resolve(value) {
		...
		if (!this.submitted) this.markSubmitted({
			state: value.state,
			...value.resultId === void 0 ? {} : { resultId: value.resultId }
		});
...
function parseMinerUJobRecord(input) {
	...
	if (sourceFiles.length !== request.files.length || files.length !== request.files.length) throw new TypeError("MinerUJobRecord source, request, and status file counts must match");
	for (let index = 0; index < request.files.length; index++) {
		...
		if (source.fileId !== requested.fileId || source.name !== requested.name || source.bytes !== requested.bytes || source.sha256 !== requested.sha256) throw new TypeError("MinerUJobRecord sourceFiles must exactly match request.files");
		if (statusFile.fileId !== requested.fileId || statusFile.name !== requested.name) throw new TypeError("MinerUJobRecord file statuses must match request.files");
	}
	const ref = resolution.kind === "cache-hit" ? void 0 : resolution.ref;
	if (ref !== void 0) {
		if (ref.provider !== providerId || ref.files.length !== request.files.length) throw new TypeError("MinerUJobRecord provider reference does not match its provider or request");
		const dataIds = /* @__PURE__ */ new Set();
		for (let index = 0; index < ref.files.length; index++) {
			const mapped = ref.files[index];
			const requested = request.files[index];
			if (mapped.fileId !== requested.fileId || mapped.name !== requested.name || dataIds.has(mapped.dataId)) throw new TypeError("MinerUJobRecord provider file mappings must uniquely match request.files");
...
var JobRepository = class {
	...
	async create(session, job) {
		...
		if (validated.request.files.length !== 1 || validated.sourceFiles.length !== 1 || validated.files.length !== 1) throw new TypeError("New MinerU jobs must contain exactly one file");
...
const MAX_PERSISTED_JOB_BYTES = 1048576;
...
async function readPersistedJobBounded(path, signal) {
	const stream = createReadStream(path);
	const onAbort = () => {
		stream.destroy(signal?.reason instanceof Error ? signal.reason : new DOMException("Aborted", "AbortError"));
	};
...
async function makeQuarantineTreeWritable(root, signal) {
	const prepare = async (path, depth) => {
		signal?.throwIfAborted();
		...
		for (const entry of directory.entries) {
			...
			if (!await prepare(join(path, entry.name), depth + 1)) return false;
		}
		try {
...
var StorageMaintenanceService = class {
	...
	results;
	lock;
	constructor(paths, results, lock) {
		this.paths = paths;
	...
	async getStatistics(signal) {
		this.assertLockHeld();
		const [publishedUsage, jobsUsage, stagingUsage, quarantineUsage, publishedCount, jobsCount, stagingCount, quarantineCount] = await Promise.all([
	...
	async cleanupQuarantine(options) {
		...
		for (const entryId of entryIds) {
			...
			if (!dryRun) try {
				if (!await makeQuarantineTreeWritable(entryPath, options.signal)) {
					skippedCount++;
					continue;
				}
				await rm(entryPath, {
	...
	async countPersistedJobFiles(signal) {
		let count = 0;
		...
		for (const sessionEntry of jobs.entries) {
			...
			for (const jobEntry of sessionDirectory.entries) {
				...
				const jobId = jobEntry.name.slice(0, -5);
				try {
					asJobId(jobId);
				} catch {
					continue;
	...
	async collectJobReferences(signal, diagnostics) {
		...
		if (jobs.kind !== "entries") {
			...
			return {
				...
				report: {
					complete,
					scannedJobCount,
					referencedCacheKeyCount: cacheKeys.size,
					malformedJobCount,
					unreadableJobCount,
					unsafeJobEntryCount
				}
		...
		for (const sessionEntry of jobs.entries) {
			...
			for (const jobEntry of sessionDirectory.entries) {
				...
				if (!(jobEntry.name.startsWith("mj_") && jobEntry.name.endsWith(".json") && !jobEntry.name.includes(".tmp."))) {
					unsafeJobEntryCount++;
					complete = false;
					addDiagnostic(diagnostics, "persisted-jobs", jobEntry.name, "unexpected-entry");
					continue;
				}
				const idText = jobEntry.name.slice(0, -5);
				let jobId;
				try {
	...
	addJobReferences(record, cacheKeys) {
...
const fileStatusSchema = {
	...
	properties: {
		...
		job_id: {
			type: "string",
			description: "Per-file MinerU job ID for a multi-file submission."
		},
		progress: {
			type: "object",
...
const resultFileViewSchema = {
	...
	properties: {
		...
		job_id: {
			type: "string",
			description: "Per-file MinerU job ID in a folded multi-file result."
		},
		state: {
			type: "string",
			description: "Per-file final state in a folded multi-file result."
		},
		result_id: {
			type: "string",
			description: "Published single-file result ID."
		},
		manifest_path: {
			type: "string",
			description: "Published single-file manifest path."
		},
		cache_hit: {
			type: "boolean",
			description: "Whether this individual file result came from cache."
		},
		failure: failureSchema,
		artifacts_truncated: {
			type: "boolean",
...
const submitChildSchema = {
	type: "object",
	properties: submitOutputProperties,
	additionalProperties: false
};
const parseChildSchema = {
	type: "object",
	properties: {
		...submitOutputProperties,
		...
		files: {
			...
			items: {
				type: "object",
				properties: {
					...resultFileViewSchema.properties,
					state: { type: "string" },
					progress: {
						type: "object",
						properties: {
...
function renderSubmit(value) {
	if ("kind" in value) {
		...
		return [{
			type: "text",
			text: clampRenderText(`**MinerU Batch Submitted**
- State: ${value.state}
...
${sections.join("\n\n")}`)
	...
	if (value.files.length > 0) {
		lines.push("- Files:");
		for (const file of value.files) lines.push(`  - ${file.name} (${file.state})${file.job_id === void 0 ? "" : ` [job: \`${file.job_id}\`]`}`);
	}
...
function renderStatus(value) {
	...
	if (value.files.length > 0) {
		...
		for (const file of value.files) {
			...
			lines.push(`  - ${file.name}: ${file.state}${progress}${file.job_id === void 0 ? "" : ` [job: \`${file.job_id}\`]`}`);
		}
	}
...
function renderResult(value) {
	...
	if (value.files.length > 0) {
		...
		for (const file of value.files) {
			...
			if (file.job_id !== void 0) lines.push(`  - Job: \`${file.job_id}\``);
			if (file.state !== void 0) lines.push(`  - State: ${file.state}`);
			if (file.result_id !== void 0) lines.push(`  - Result: \`${file.result_id}\``);
			if (file.manifest_path !== void 0) lines.push(`  - Manifest: \`${file.manifest_path}\``);
			if (file.failure !== void 0) lines.push(`  - Failure: [${file.failure.code}] ${file.failure.message}`);
...
function requireAgentSession(exec) {
	...
	if (session === void 0) throw new MinerUError(failure("UNAUTHENTICATED_SESSION", "MinerU operations require an authenticated agent session (UNAUTHENTICATED_SESSION)"));
	return {
		session,
		signal: exec.signal
	};
...
function requireJobId(args) {
	...
	for (const key of Object.keys(args)) if (key !== "job_id") throw new MinerUError(failure("INVALID_REQUEST", `Unsupported tool argument ${key}`));
	const { job_id: jobId } = args;
	if (typeof jobId !== "string" || jobId.trim() === "") throw new MinerUError(failure("INVALID_REQUEST", "job_id is required"));
	return jobId;
}
function registerTools(ctx, getService) {
	const disposers = [];
			...
			schema: {
				...
				properties: {
					kind: {
						type: "string",
						enum: ["batch"],
						description: "Present only for a multi-file batch envelope."
					},
					jobs: {
						type: "array",
						description: "Independent per-file submission jobs.",
						items: submitChildSchema
					},
					job_id: {
						type: "string",
						description: "Unique MinerU job ID (mj_...)."
		...
		output: {
			schema: {
				...
				properties: {
					...
					jobs: {
						type: "array",
						description: "Independent per-file parse views.",
						items: parseChildSchema
					},
					job_id: {
						type: "string",
						description: "MinerU job ID."
					...
					files: {
						...
						items: {
							...
							properties: {
								...
								job_id: {
									type: "string",
									description: "Per-file MinerU job ID in a multi-file response."
								},
								state: {
									type: "string",
...
async function apply(ctx, entryConfig = {}) {
	...
	const validateRuntimeConfig = (value) => {
		...
		if (next.storage.storageRoot !== fixedStorageRoot) throw new TypeError("storage.storageRoot cannot change while the MinerU plugin is running");
		return next;
	};
