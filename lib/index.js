...
function narrowPageSelection(requested, totalPages) {
	...
	if (requested === void 0) return {
		pagesSet: void 0,
		pagesLabel: maxBound > 1 ? `1-${maxBound}` : "1"
	};
	const valid = /* @__PURE__ */ new Set();
	for (const p of requested) if (p >= 1 && p <= maxBound) valid.add(p);
	if (valid.size === 0) {
		if (Math.min(...requested) > maxBound) valid.add(maxBound);
		else valid.add(1);
	}
	...
	return {
		pagesSet: valid,
		pagesLabel: intervals.map(({ start, end }) => start === end ? String(start) : `${start}-${end}`).join(",")
	};
}
...
function formatResultProse(value) {
	...
	if (status === "complete") footer = "\n---\n[Status: Content complete. " + pagesInfo + "Full requested document markdown delivered above.]";
		...
		const mdGuidance = mdPath !== void 0 ? "Full markdown artifact at: " + mdPath + resumeInfo + "." : "Full markdown artifact path unavailable.";
	...
	} else if (value.ordered_images && value.ordered_images.length > 0) for (let idx = 0; idx < value.ordered_images.length; idx++) {
