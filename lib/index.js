...
function normalizeFocusSelection(input) {
	...
	if (typeof input === "string") {
		...
		return /* @__PURE__ */ new Set([trimmed]);
	}
	if (Array.isArray(input)) {
...
function resolvePaths(input, maxFiles) {
	const paths = [];
	if (typeof input.file_path === "string" && input.file_path.trim() !== "") paths.push(input.file_path.trim());
	if (Array.isArray(input.file_paths)) {
		for (const path of input.file_paths) if (typeof path === "string" && path.trim() !== "" && !paths.includes(path.trim())) paths.push(path.trim());
	}
...
var ProviderHttpClient = class {
	...
	async requestJson(optionsOrMethod, path, body, headers, context, acceptedStatuses, options) {
		const opts = typeof optionsOrMethod === "string" ? {
...
*/
async function executeJsonRequest(options) {
	return await (options.client ?? new ProviderHttpClient({
	...
	async execute() {
		...
		try {
			...
			while (snapshot.files.some((file) => file.state !== "completed" && file.state !== "failed")) {
				await setTimeout$1(this.options.pollIntervalMs, void 0, { signal: this.controller.signal });
...
function extractBlocksMarkdown(contentList, pagesSet, focusSet, imageArtifacts) {
	...
	for (const block of contentList) {
		...
		if (pagesSet !== void 0 && !pagesSet.has(pageNum)) continue;
		...
		if (cat === "image") {
			...
			const base = rawPath ? basename(String(rawPath)).toLowerCase() : void 0;
...
function allocateReclaimedShares(lengths, totalBudget) {
	const result = new Array(lengths.length).fill(0);
	const active = lengths.map((len, idx) => ({
		idx,
		len
	}));
	...
	while (active.length > 0) {
		...
		if (fitIndex !== -1) {
			...
			for (const item of active) result[item.idx] = share;
			break;
		}
	}
	return result;
}
async function readMarkdownFile(path, totalBytes, maxCharsToRead) {
	...
	try {
		...
		return {
			...
			isCompleteFile: bytesRead >= totalBytes
		};
	} finally {
		await handle.close();
	}
...
function formatResultProse(value) {
	...
	const lines = ["**MinerU Parse Result** (Source: " + value.source + ", Cache: " + (value.cache_hit ? "hit" : "miss") + ")"];
	...
	if (status === "complete") footer = "\n---\n[Status: Content complete. Full document markdown delivered above. Artifact path: " + value.manifest_path + "]";
		...
		const resumeInfo = value.read_offset_line !== void 0 ? " (resume line: offset=" + String(value.read_offset_line) + ")" : "";
		footer = "\n---\n[Status: Content partial (truncated to output limit). " + (mdPath !== void 0 ? "Full markdown artifact at: " + mdPath + resumeInfo + "." : "Full markdown artifact path unavailable.") + " Manifest: " + value.manifest_path + "]";
	} else footer = "\n---\n[Status: Markdown content was not requested. Manifest: " + value.manifest_path + "]";
	lines.push(footer);
	return lines.join("\n");
}
function formatParseDocumentProse(value) {
	if (!("kind" in value)) return formatResultProse(value);
	const sections = value.results.map((result) => result.state === "completed" ? formatResultProse(result) : "**" + result.name + "**: [" + result.failure.code + "] " + result.failure.message);
...
function formatSingleSummaryProse(value) {
	...
	if (outline && outline.length > 0) {
		...
		for (const heading of outline) {
			const indent = "  ".repeat(Math.max(0, heading.level - 1));
			const pageInfo = heading.page !== void 0 ? ` (Page ${String(heading.page)})` : ` (line ${String(heading.line)})`;
			lines.push(`${indent}- ${heading.title}${pageInfo}`);
		}
	}
	lines.push("", "---", "💡 **Next Steps**: The document has been fully parsed and cached in local storage. Use `read_pdf` to inspect content on demand:", `- Read specific pages: \`read_pdf({ file_path: "${fileName}", pages: "1-3" })\``, `- Focus on tables: \`read_pdf({ file_path: "${fileName}", focus: "table" })\``, `- Focus on figures/images: \`read_pdf({ file_path: "${fileName}", focus: "image" })\``, `- Read complete text: \`read_pdf({ file_path: "${fileName}" })\``);
	return lines.join("\n");
}
function formatParseSummaryProse(value) {
	if ("kind" in value && value.kind === "batch") {
		...
		for (const result of value.results) if (result.state === "completed") lines.push(formatSingleSummaryProse(result));
		else lines.push(`**${result.name}**: [${result.failure.code}] ${result.failure.message}`);
		return lines.join("\n\n");
	}
...
var MinerUService = class {
	...
	async projectSingle(data, limit) {
		...
		if (contentListArtifact) try {
			...
			else if (Array.isArray(parsed?.list)) contentList = parsed.list;
			else if (Array.isArray(parsed?.content_list)) contentList = parsed.content_list;
		} catch {
			contentList = void 0;
		}
		...
		if (contentList && contentList.length > 0) {
			...
			if (!raw.isCompleteFile && data.markdownPath) try {
		...
		let view = {
			...
			content_status: contentStatus,
			markdown_path: data.markdownPath,
			...readOffsetLine !== void 0 ? { read_offset_line: readOffsetLine } : {},
			manifest_path: data.manifestPath,
			output_limit_chars: limit,
			markdown_content: content,
			ordered_images: orderedImages,
	...
	async projectBatch(rawItems, limit) {
		const completedItems = rawItems.filter((i) => i.state === "completed");
		const itemExtractions = /* @__PURE__ */ new Map();
		for (const item of completedItems) if (item.markdownRequested) {
			...
			if (contentList && contentList.length > 0) {
				let rawFallback = "";
				if (item.markdownPath) try {
					rawFallback = await readFile(item.markdownPath, "utf8");
				} catch {}
		...
		const skeletonResults = rawItems.map((item) => {
			if (item.state === "failed") return item.failureView;
			const markdownArtifact = item.markdownPath ? {
				kind: "markdown",
		...
		const batchState = completedCount === rawItems.length ? "completed" : completedCount === 0 ? "failed" : "partially-completed";
		let batchSkeleton = {
			kind: "batch",
			state: batchState,
			output_limit_chars: limit,
		...
		const shares = allocateReclaimedShares(mdItems.map((i) => itemExtractions.get(i.fileId)?.fullSourceText.length ?? 0), totalTextBudget);
		const finalResults = await Promise.all(rawItems.map(async (item) => {
			if (item.state === "failed") return item.failureView;
			if (!item.markdownRequested) return {
				state: "completed",
				source: item.item.source,
				cache_hit: item.item.source === "cache",
				result_id: item.manifest.id,
			...
			if (fullSourceText.length <= share) {
				...
				content = cut.text;
				readOffsetLine = cut.resumeLine;
				if (item.secondaryArtifacts.length > 0 || baseArtifactsStripped) artifactsTruncated = true;
				if (!toc || toc.length === 0) toc = extractMarkdownHeadings(fullSourceText);
			}
		...
		let batchContentStatus;
		const completedResults = finalResults.filter((r) => r.state === "completed");
		if (completedResults.length === 0 || completedResults.every((r) => r.content_status === "not_requested")) batchContentStatus = "not_requested";
		else if (completedResults.some((r) => r.content_status === "partial")) batchContentStatus = "partial";
		else batchContentStatus = "complete";
		let batchView = {
			kind: "batch",
			state: batchState,
			output_limit_chars: limit,
		...
		while (JSON.stringify(batchView).length > limit || formatParseDocumentProse(batchView).length > limit) {
			...
			const strippedExtrasResults = batchView.results.map((r) => {
				if (r.state === "completed" && (r.summary !== void 0 || r.ordered_images !== void 0 && r.ordered_images.length > 0)) {
			...
			if (strippedAnyExtras) {
				batchView = {
					...batchView,
					results: strippedExtrasResults
				};
				continue;
			}
			const partialCandidates = batchView.results.filter((r) => r.state === "completed" && r.content_status === "partial" && Boolean(r.markdown_content && r.markdown_content.length > 0));
			const candidates = partialCandidates.length > 0 ? partialCandidates : batchView.results.filter((r) => r.state === "completed" && Boolean(r.markdown_content && r.markdown_content.length > 0));
			if (candidates.length > 0) {
				...
				const tocCandidates = batchView.results.filter((r) => r.state === "completed" && Boolean(r.toc && r.toc.length > 0));
	...
	async parseDocument(session, input, signal, pollTimeoutMs) {
		...
		for (let index = 0; index < outcomes.length; index++) {
			...
			const file = item.prepared.request.files[0];
			if (outcome.state === "failed" || outcome.resultId === void 0) {
				rawItems.push({
					state: "failed",
					failureView: {
						state: "failed",
			...
			if (manifest === void 0 || manifest.id !== outcome.resultId) {
				rawItems.push({
					state: "failed",
					failureView: {
						state: "failed",
			...
			if (markdownRequested && markdownRef === void 0) {
				rawItems.push({
					state: "failed",
					failureView: {
						state: "failed",
			...
			const markdownPath = markdownRef !== void 0 ? this.options.results.resolveArtifactAbsolutePath(item.cacheKey, markdownRef.relativePath) : void 0;
...
function parseLockPayload(raw) {
	try {
		...
		const record = value;
		if (!Number.isSafeInteger(record.pid) || record.pid <= 0 || typeof record.ownerToken !== "string" || record.ownerToken.length === 0 || !Number.isSafeInteger(record.createdAt) || record.createdAt < 0 || typeof record.hostname !== "string" || record.hostname.length === 0) return null;
		return {
			pid: record.pid,
...
var ProcessLock = class {
	...
	*/
	async withLock(operation, signal) {
		...
		if (!wasHeld) await this.acquire(signal);
		try {
			return await operation();
		} finally {
			if (!wasHeld) await this.release();
		}
	}
	async acquire(signal) {
		...
		await chmod(this.paths.root, 448).catch(() => void 0);
		signal?.throwIfAborted();
		const payload = {
			pid: process.pid,
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			signal?.throwIfAborted();
			try {
				await writeFile(this.lockFilePath, serialized, {
					flag: "wx",
					mode: 384
				});
				await chmod(this.lockFilePath, 384).catch(() => void 0);
				if (signal?.aborted) {
				...
				if (!existing) throwMinerU("STORAGE_LOCKED", "MinerU storage lock metadata is invalid or ambiguous");
				if (existing.pid === process.pid && existing.ownerToken === this.ownerToken) {
					this.acquired = true;
					return;
				}
				if (existing.hostname.toLowerCase() !== hostname().toLowerCase()) throwMinerU("STORAGE_LOCKED", "MinerU storage is locked by another host");
				try {
					process.kill(existing.pid, 0);
					if (attempt < 2) {
						await setTimeout$1(25, void 0, { signal }).catch(() => void 0);
						continue;
					}
					throwMinerU("STORAGE_LOCKED", "MinerU storage is already locked by an active process");
				} catch (killError) {
					if (killError?.failure?.code === "STORAGE_LOCKED") throw killError;
					if (killError.code === "ESRCH") {
						try {
							await unlink(this.lockFilePath);
						} catch (unlinkError) {
							if (unlinkError.code !== "ENOENT") throwMinerU("STORAGE_LOCKED", "MinerU stale storage lock could not be removed");
						}
						continue;
					}
	...
	async release() {
		...
		try {
			...
			if (existing && existing.ownerToken === this.ownerToken && existing.pid === process.pid) await unlink(this.lockFilePath).catch(() => void 0);
		} catch {}
	}
};
//#endregion
...
var ResultRepository = class {
	...
	async commitTransaction(tx, manifest, signal) {
		...
		if (this.lock !== void 0) return await this.lock.withLock(doCommit, signal);
	...
	*/
	...
	async quarantine(sourcePath, reason = "quarantine") {
		...
		if (this.lock !== void 0) return await this.lock.withLock(doQuarantine);
...
const confirmationToken = cacheClearConfirmationToken;
...
var StorageMaintenanceService = class {
	...
	static confirmationToken(cacheKeys) {
	...
	async scanIntegrityInternal(options) {
	...
	acquireDestructiveAccess() {
		...
		const releaseExclusive = this.accessGate.tryAcquireExclusive();
		if (releaseExclusive === void 0) throwMinerU("STORAGE_LOCKED", "MinerU storage is in use by an active reader");
...
const REMOVED_TOOL_PARAMETERS = /* @__PURE__ */ new Set([
	"model",
	"ocr",
	"formula",
	"table",
...
function assertNoRemovedParameters(args) {
	for (const key of Object.keys(args)) if (REMOVED_TOOL_PARAMETERS.has(key)) throw new MinerUError(failure("INVALID_REQUEST", `Unsupported parameter: ${key}. Technical parameters (model, ocr, formula, table, language, artifacts, max_inline_images) have been removed from tool arguments.`));
}
function extractFilePaths(args) {
	...
	if (Array.isArray(args.file_paths)) {
		for (const p of args.file_paths) if (typeof p === "string" && p.trim() !== "" && !paths.includes(p.trim())) paths.push(p.trim());
	}
	if (paths.length === 0) throw new MinerUError(failure("INVALID_REQUEST", "Exactly one local document path is required"));
	return paths;
}
function parseAsyncInput(args) {
	...
	return { input: {
		file_paths: extractFilePaths(obj),
		...typeof obj.file_path === "string" && obj.file_path.trim() !== "" ? { file_path: obj.file_path } : {}
	} };
}
...
function renderResult(value) {
	...
	}, ...(value.inlined_images ?? []).flatMap((img) => img.attachmentRef ? [{
		type: "image",
...
function renderParseDocument(value) {
	...
	const limit = "output_limit_chars" in value && typeof value.output_limit_chars === "number" && Number.isSafeInteger(value.output_limit_chars) && value.output_limit_chars > 0 ? value.output_limit_chars : DEFAULT_RENDER_LIMIT;
	return [{
		type: "text",
		text: clampRenderText(formatParseDocumentProse(value), limit)
	}, ...value.results.filter((r) => r.state === "completed").flatMap((r) => (r.inlined_images ?? []).flatMap((img) => img.attachmentRef ? [{
		type: "image",
		attachment: img.attachmentRef
	}] : []))];
}
...
async function collectImageCandidates(view) {
	const candidates = [];
	const seenPaths = /* @__PURE__ */ new Set();
	for (const file of view.files ?? []) for (const artifact of file.artifacts ?? []) if (artifact.kind === "images") {
	...
	if (candidates.length === 0 && view.manifest_path) try {
		const manifestRaw = await readFile(view.manifest_path, "utf8");
		const manifest = JSON.parse(manifestRaw);
		const manifestDir = dirname(view.manifest_path);
		for (const file of manifest.files ?? []) for (const artifact of file.artifacts ?? []) if (artifact.kind === "images") {
...
async function inlineImagesForSingleResult(view, attachments, signal) {
	let candidates = [];
	if (view.ordered_images && view.ordered_images.length > 0) candidates = view.ordered_images.map((img) => ({
		path: img.path,
		bytes: img.bytes,
		name: img.name,
		mediaType: img.media_type
	}));
	else candidates = await collectImageCandidates(view);
...
async function processInlineImages(view, attachments, signal) {
	if (!("kind" in view)) return await inlineImagesForSingleResult(view, attachments, signal);
	const updatedResults = [];
	for (const res of view.results) if (res.state === "completed") {
		const updated = await inlineImagesForSingleResult(res, attachments, signal);
		updatedResults.push(updated);
	} else updatedResults.push(res);
	return {
		...view,
...
function registerTools(ctx, getService, accessGate) {
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		output: {
			...
			presentationMeta: (_args, value) => {
				const doc = value;
				if ("kind" in doc && doc.kind === "batch") return {
					kind: "batch",
					state: doc.state,
		...
		execute: async (args, exec) => {
			...
			const shouldInline = inline_images !== false && supportsImage && attachments !== void 0;
			const rawResult = await withStorageAccess(() => getService().parseDocument(agent.session, input, exec.signal, pollTimeoutMs));
			if (shouldInline && attachments) return await processInlineImages(rawResult, attachments, exec.signal);
			return rawResult;
		}
	})));
	return async () => {
...
function registerRpc(ctx, deps) {
	...
	const dispose = ctx.connection.rpc.handle(RPC_CHANNEL, async (endpoint, payload, signal) => {
		try {
			...
			return fail(err instanceof Error ? err.message : String(err), err instanceof TypeError ? "mineru/invalid-argument" : "mineru/internal");
		}
	}, { authority: "loopback" });
...
//#endregion
export { ARTIFACT_KINDS, CACHE_KEY_SPEC_VERSION, CANONICAL_PARSE_REQUEST_SCHEMA_VERSION, Config, DEFAULT_RETRY_POLICY, FOCUS_KINDS, MINERU_RESULT_MANIFEST_SCHEMA_VERSION, MinerUError, MinerUService, OfficialV4Provider, ProviderHttpClient, RESULT_SCHEMA_VERSION, SelfHostedV2Provider, StorageMaintenanceService, allocateReclaimedShares, apply, asCacheKey, asFileId, asOperationId, asProviderConfigId, asResultId, asSessionId, assertSafePathSegment, cacheClearConfirmationToken, calculateBackoffDelay, computeDocumentSummary, confirmationToken, createFileId, createHttpStatusError, createOperationId, createStructuredDiagnosticSink, defaultMinerUConfig, defaultProviderConfig, defaultSleep, emitDiagnostic, executeJsonRequest, executeWithRetry, extractBlocksMarkdown, extractErrorMessage, extractMarkdownHeadings, failure, fallbackExtractFromMarkdown, findMarkdownArtifactPath, formatCaption, formatParseDocumentProse, formatParseSummaryProse, formatResultProse, formatSingleSummaryProse, getBlockCategory, getRasterMediaType, inject, isRetryableError, isRetryableHttpStatus, mergeRetryOptions, migrateConfig, name, normalizeArtifactKinds, normalizeFocusSelection, normalizePageRanges, normalizePageSelection, parseRetryAfter, providerById, readBoundedResponseText, readMarkdownFile, resolveProviderUrl, resolveRetryPolicy, resultIdForCacheKey, safeStringSlice, sanitizeDiagnostic, throwMinerU, toMinerUFailure, truncateAtCleanBoundary, validateProviderCapabilities };
