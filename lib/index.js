...
var MinerUService = class {
	...
	async ensureParsed(session, input, signal) {
	...
	async resolveParsedResult(session, input, signal, pollTimeoutMs) {
		...
		execute: async (args, exec) => {
			...
			return {
				job_id: jobs.start({
					...
					owner: agent,
					run: () => {
						const done = withStorageAccess(() => getService().ensureParsed(agent.session, input, controller.signal), controller.signal).then((value) => nativeSuccessOutcome(value)).catch((error) => {
...
const Config = z.object({
	schemaVersion: z.union([z.const(1), z.const(2)]),
...
async function apply(ctx, entryConfig = {}) {
	...
	const migrateRuntimeConfig = (value) => {
		const parsed = parseConfigWithMigration(value);
		return {
			...parsed,
			config: validateParsedRuntimeConfig(parsed.config)
		};
	};
	...
	const persistMigration = async () => {
		try {
			await settings.mutate("dsh-pdf-mineru", [
				...
				{
					op: "unset",
					path: ["defaults", "artifacts"]
				},
				{
					op: "unset",
					path: ["limits", "maxFilesPerRequest"]
				}
			]);
		} catch {
	...
	const settingsScope = settings.register("dsh-pdf-mineru", Config, {
		...
		applies: "live",
		validate: (value) => {
			migrateRuntimeConfig(value);
		}
	});
	const storedConfig = migrateRuntimeConfig(settingsScope.get());
	persistedConfig = storedConfig.config;
	if (storedConfig.migrated) await persistMigration();
	const pruneLegacyBloatedSettings = async () => {
		try {
			const user = (settings.describe?.()?.find((d) => d.ns === "dsh-pdf-mineru"))?.user;
			if (typeof user !== "object" || user === null || Array.isArray(user)) return;
			const ops = detectBloatedSettingsOps(user, defaultMinerUConfig());
			if (ops.length > 0) await settings.mutate("dsh-pdf-mineru", ops);
		} catch {
	...
	ctx.effect(() => settingsScope.watch(async (next) => {
		const parsed = migrateRuntimeConfig(next);
		persistedConfig = parsed.config;
		if (parsed.migrated) await persistMigration();
