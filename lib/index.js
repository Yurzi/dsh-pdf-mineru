...
async function readMarkdownFile(path, totalBytes) {
	...
	try {
		...
		return {
			text: new TextDecoder$1("utf-8").decode(buffer),
			isCompleteFile: true
		};
	} finally {
		await handle.close();
...
var MinerUService = class {
	...
	async projectSingle(data, limit, cursorOffset = 0, cursorPayload) {
		...
		if (contentList && contentList.length > 0) {
			...
			if (narrowed.fullyOutOfRange) throw new MinerUError(failure("INVALID_REQUEST", formatPageOutOfRangeMessage(docSummary.page_count)));
		...
		if (focusSet.size === 1 && focusSet.has("artifacts")) {
			const baseFiles = [{
				...
				...data.markdownPath !== void 0 ? { markdown_path: data.markdownPath } : {}
			}];
			const candidate = {
				state: "completed",
				source: data.item.source,
		...
		let view = {
			...
			ordered_images: orderedImages,
			...docSummary !== void 0 ? { summary: docSummary } : {},
			...(contentStatus === "partial" || contentStatus === "complete") && toc !== void 0 ? { toc } : {},
			...pagesLabel !== void 0 ? { pages: pagesLabel } : {}
		};
		...
		else if (view.summary !== void 0 || view.ordered_images !== void 0 && view.ordered_images.length > 0) {
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			return await withStorageAccess(async () => {
				...
				return fitPostImageBudget(shouldInline && attachments ? await inlineImagesForSingleResult(rawResult, attachments, maxInlineImages, exec.signal) : rawResult);
			}, exec.signal);
		}
	})));
	return async () => {
