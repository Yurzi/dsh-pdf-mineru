...
function decodeReadCursor(token) {
	...
	if (obj.v !== 2) throw new TypeError("cursor is expired; re-read without a cursor");
...
var WarningCollector = class {
	...
	add(code, location, message) {
		if (this.orders !== void 0 && !this.orders.has(Number(/^block ([0-9]+)/.exec(location)?.[1]))) return;
...
const POSSIBLE_TEXT_GAP_PATTERNS = [/中的\s+个/u, /(?:包含|通过率为|分别通过)\s+(?:个|和|，|；|。)/u];
...
function normalizeDocumentBlocks(contentList, resultId, warningBlockOrders) {
	...
	return {
		blocks: contentList.map((source, index) => {
			...
			} else if (typeof source.type === "string" && !KNOWN_BLOCK_TYPES.has(source.type.toLowerCase())) warnings.add("DOCUMENT_INDEX_UNKNOWN_BLOCK_TYPE", blockLocation, "unknown top-level block type was preserved");
...
function deliverReadChunk(options) {
	...
	for (;;) {
		...
		const warnings = boundedWarnings([...base.warnings ?? [], ...chunkImages.length > 20 ? ["[VISUAL_METADATA_SHORTENED] Use block_id to inspect images not listed in this chunk."] : []]);
		const view = {
			...base,
...
var MinerUService = class {
	...
	async projectSingle(data, limit, cursorPayload, input = {}, signal) {
		...
		const warningOrders = rawPagesSet === void 0 && blockId === void 0 ? void 0 : new Set(rawBlocks.flatMap((block, index) => (rawPagesSet === void 0 || typeof block.page_idx === "number" && rawPagesSet.has(block.page_idx + 1)) && (blockId === void 0 || blockId === data.manifest.id + ":b" + (index + 1)) ? [index + 1] : []));
		...
		return deliverReadChunk({
			...
			signal,
			identity: JSON.stringify(data.manifest.files),
			selection: {
