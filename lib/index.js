...
var MinerUService = class {
	...
	async submit(session, input, signal) {
		...
		if (current.storage.cacheEnabled) {
			...
			if (hit !== void 0) {
				...
				this.diagnostic({
					...
					jobId: job.id,
					bytes: prepared.request.files.reduce((total, source) => total + source.bytes, 0),
					cacheHit: true
				});
		...
		const acquired = this.options.operations.acquire(cacheKey, active.config.id, current.polling.operationTimeoutMs, (operation) => this.runOperation(operation, prepared, active, compatibility));
...
const Config = import_lib.default.object({
	...
	retry: import_lib.default.object({
		maxAttempts: import_lib.default.number(),
		baseDelayMs: import_lib.default.number(),
		maxDelayMs: import_lib.default.number()
	}),
