...
var StorageMaintenanceService = class {
	...
	lock;
	accessGate;
	constructor(paths, results, lock, accessGate = new StorageAccessGate()) {
		this.paths = paths;
	...
	async clearCacheInternal(options, exclusiveAcquired, activeAccessCount) {
		...
		const preflightEligible = references.report.complete && references.activeJobCount === 0 && activeAccessCount === 0 && traversal.complete && !traversal.truncated && unsafeResultCount === 0;
...
function registerTools(ctx, getService, accessGate) {
	...
	disposers.push(ctx.tools.register(defineTool({
		...
		execute: async (args, exec) => {
			...
			return await withStorageAccess(() => getService().submit(session, args, signal));
		}
	})));
	disposers.push(ctx.tools.register(defineTool({
...
async function apply(ctx, entryConfig = {}) {
	...
	try {
		...
		await results.cleanupStaging(persistedConfig.storage.stagingTtlMs, operations.activeOperationIds(), startup.signal);
		startup.signal.throwIfAborted();
