...
async function invokeProcess(runProcess, request) {
	try {
		...
		if (errorCode(error) === "ENOENT") throw new MinerUError(failure("UNSUPPORTED_OPTION", "Local PDF page rendering requires the " + request.command + " Poppler executable"), { cause: error });
...
function pngDimensions(data) {
	...
	if (bytes.byteLength < 24 || !bytes.subarray(0, 8).equals(PNG_SIGNATURE) || bytes.readUInt32BE(8) !== 13 || bytes.toString("ascii", 12, 16) !== "IHDR") throw invalid("pdftoppm returned an invalid PNG");
	const width = bytes.readUInt32BE(16);
	const height = bytes.readUInt32BE(20);
	if (width < 1 || height < 1 || width > 1600 || height > 2e3 || width * height > 32e5) throw new MinerUError(failure("RESULT_TOO_LARGE", "Rendered PNG dimensions exceed the page-rendering limit"));
	return {
		width,
		height
	};
	...
	return async (input) => {
		...
		try {
			...
			const infoRequest = {
				...
				timeoutMs: Math.min(1e4, runtimeMs),
			...
			const renderRequest = {
				...
				cwd: temporaryDirectory,
				signal: operation.signal,
				timeoutMs: Math.min(3e4, runtimeMs),
			...
			if ((await invokeProcess(runProcess, renderRequest)).exitCode !== 0) throw invalid("pdftoppm could not render the requested PDF page");
			operation.signal.throwIfAborted();
			let data;
			try {
