...
function formatTocMarkdown(headings, options) {
	...
	for (const heading of headings) {
		...
		const location = heading.page !== void 0 ? ` (Page ${String(heading.page)})` : ` (line ${String(heading.line)})`;
...
function formatSingleSummaryProse(value) {
	...
	lines.push("", "---", "💡 **Next Steps**: The document has been fully parsed and cached in local storage. Use `read_pdf` to inspect content on demand:", `- Read specific pages: \`read_pdf({ file_path: "${fileName}", pages: "1-3" })\``, `- Focus on tables: \`read_pdf({ file_path: "${fileName}", focus: "table" })\``, `- Focus on figures/images: \`read_pdf({ file_path: "${fileName}", focus: "image" })\``, `- Inspect outline / TOC: \`read_pdf({ file_path: "${fileName}", focus: "toc" })\``, `- Read complete text: \`read_pdf({ file_path: "${fileName}" })\``);
	return lines.join("\n");
}
...
var MinerUService = class {
	...
	async projectSingle(data, limit) {
		...
		if (focusSet.has("toc")) {
			...
			const tocMd = formatTocMarkdown(filteredToc, { pageRange: typeof data.inputPages === "string" ? data.inputPages : Array.isArray(data.inputPages) ? data.inputPages.join(",") : data.inputPages !== void 0 ? String(data.inputPages) : void 0 });
...
function nativeSuccessOutcome(value) {
	return {
		status: "completed",
		detail: "completed",
		output: formatParseSummaryProse(value)
	};
}
