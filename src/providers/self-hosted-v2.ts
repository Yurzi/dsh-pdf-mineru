...
export class SelfHostedV2Provider implements MinerUProvider {
  ...
  async compatibilityKey(
    ...
    const rawVersion = context.configuredVersion ?? this.config.configuredVersion ?? this.config.id ?? 'v2'
    const safeVersion = rawVersion.replace(/[^A-Za-z0-9._-]/g, '_')
    return `self-hosted-v2:${urlHash}:${safeVersion}:${request.semantics.model}`
  }
  ...
  async submit(
    ...
    const fileSnapshots: ProviderFileSnapshot[] = request.files.map(f => ({
      ...
      failure: state === 'failed'
        ? failure('REMOTE_PARSE_FAILED', data.error ?? 'Remote task submission failed', false, { provider: 'self-hosted-v2', fileId: f.fileId })
        : undefined,
    }))
  ...
  async inspect(ref: ProviderJobRef, context: ProviderCallContext): Promise<ProviderJobSnapshot> {
    ...
    const fileFailure = state === 'failed'
      ? failure('REMOTE_PARSE_FAILED', data.error ?? 'Remote task failed', false, { provider: 'self-hosted-v2' })
      : undefined
    ...
    for (const file of ref.files) {
      ...
      if (artifacts.length === 0) {
        collectedFiles.push({
          ...
          name: file.name,
          artifacts: [],
          failure: failure('REMOTE_PARSE_FAILED', `No matching artifacts produced for file "${file.name}"`, false, { provider: 'self-hosted-v2', fileId: file.fileId }),
        })
      } else {
        collectedFiles.push({
          fileId: file.fileId,
  ...
  private async requestJson<T>(
    ...
    try {
      ...
      try {
        ...
        if (timedOut) {
          throw new MinerUError(failure('PROVIDER_UNAVAILABLE', `Request to MinerU server timed out after ${context.timeoutMs}ms`, true))
        }
      ...
      if (!acceptedStatuses.includes(status)) {
        ...
        try {
          ...
          // ignore error reading body on failure status
        }
        ...
        if (status === 401 || status === 403) {
          throw new MinerUError(failure('AUTHENTICATION_FAILED', `Authentication failed (${status})${diagnostic}`, false, { provider: 'self-hosted-v2' }))
        }
        if (status === 404) {
          throw new MinerUError(failure('JOB_NOT_FOUND', `Resource not found (${status})${diagnostic}`, false, { provider: 'self-hosted-v2' }))
        }
        if (status === 413) {
          throw new MinerUError(failure('FILE_TOO_LARGE', `Uploaded file is too large (${status})${diagnostic}`, false, { provider: 'self-hosted-v2' }))
        }
        if (status === 429) {
          throw new MinerUError(failure('PROVIDER_RATE_LIMITED', `Provider rate limit exceeded (${status})${diagnostic}`, true, { provider: 'self-hosted-v2' }))
        }
        if (status >= 500) {
          throw new MinerUError(failure('PROVIDER_UNAVAILABLE', `MinerU server error (${status})${diagnostic}`, true, { provider: 'self-hosted-v2' }))
        }
      ...
      while (true) {
        ...
        if (value) {
          ...
          if (totalBytes > maxBytes) {
            throw new MinerUError(
              failure('RESULT_TOO_LARGE', `Response body exceeded maximum allowed size of ${maxBytes} bytes`),
            )
          }
          chunks.push(value)
        }
